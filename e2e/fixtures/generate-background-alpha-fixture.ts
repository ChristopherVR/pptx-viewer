/**
 * Generates `background-alpha.pptx`: a one-slide deck whose background is a
 * semi-transparent `a:solidFill` (`<a:srgbClr val="CEE0F3"><a:alpha
 * val="43211"/></a:srgbClr>`), for `background-alpha-solid-fill.spec.ts`.
 *
 ...
 * `a:solidFill`, which nothing in the existing corpus authors. PowerPoint
 * composites a semi-transparent slide background over white, so the expected
 * rendered colour is `#EAF2FA` (blend of #CEE0F3 at 43.211% opacity onto
 * white), the same value asserted in
