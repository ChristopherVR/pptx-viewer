/**
 * Generates `hyperlink-action.pptx`: a one-slide deck holding a single
 * rectangle whose `p:cNvPr` carries a real `a:hlinkClick` (an external URL,
 * via a `TargetMode="External"` relationship), for `save-as-ppt.spec.ts`'s
 * hyperlink round-trip case.
 *
 * WHY a generated fixture rather than driving the Hyperlink dialog through
 * the UI: `HyperlinkEditDialog.tsx` (and its Vue/Angular/Svelte/Vanilla
 * ports) carries no shared ARIA contract the way the File backstage's
 * `role="dialog"` does, so a UI-driven flow here would need five different,
 * unverified selector sets instead of the one neutral upload+save flow every
 * other `save-as-*.spec.ts` already uses. Authoring the hyperlink directly in
 * the fixture keeps the spec proving what it is actually meant to prove: that
 * the `.ppt` writer/reader round-trips a hyperlink that was already on the
 * loaded deck, independent of how it got there.
 *
 * Run with: bun run e2e/fixtures/generate-hyperlink-action-fixture.ts
 */
...
export const HYPERLINK_TARGET_URL = 'https://example.com/hyperlink-fixture';
export const HYPERLINK_SHAPE_TEXT = 'Linked Shape';
...
const shape = `<p:sp><p:nvSpPr><p:cNvPr id="2" name="${HYPERLINK_SHAPE_TEXT}"><a:hlinkClick xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" r:id="rId2"/></p:cNvPr><p:cNvSpPr/><p:nvPr/></p:nvSpPr><p:spPr><a:xfrm><a:off x="${
	40 * EMU_PER_PT
}" y="${40 * EMU_PER_PT}"/><a:ext cx="${300 * EMU_PER_PT}" cy="${
	80 * EMU_PER_PT
}"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom><a:solidFill><a:srgbClr val="4472C4"/></a:solidFill></p:spPr><p:txBody><a:bodyPr/><a:lstStyle/><a:p><a:r><a:rPr lang="en-US"/><a:t>${HYPERLINK_SHAPE_TEXT}</a:t></a:r></a:p></p:txBody></p:sp>`;
...
zip.file(
	...
	`<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<p:sld xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main"><p:cSld><p:spTree><p:nvGrpSpPr><p:cNvPr id="1" name=""/><p:cNvGrpSpPr/><p:nvPr/></p:nvGrpSpPr><p:grpSpPr/>${shape}</p:spTree></p:cSld><p:clrMapOvr><a:masterClrMapping/></p:clrMapOvr></p:sld>`,
);
zip.file(
	'ppt/slides/_rels/slide1.xml.rels',
	`<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">${rel('rId1', 'slideLayout', '../slideLayouts/slideLayout1.xml')}${rel('rId2', 'hyperlink', HYPERLINK_TARGET_URL, true)}</Relationships>`,
);

const out = fileURLToPath(new URL('./hyperlink-action.pptx', import.meta.url));
