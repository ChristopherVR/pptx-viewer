/**
 * Generates `image-fill-rot-with-shape.pptx`: a one-slide deck with two
 * `<p:sp>` trapezoids (`a:custGeom`), each `flipV="1"` and each filled with the
 * SAME picture via `a:blipFill`. The only difference between them is
 * `a:blipFill/@rotWithShape`:
 *
 *  - shape 1 ("RotWithShape false"): `rotWithShape="0"` - the picture inside
 *    must stay upright/fixed to the page frame while the trapezoid outline
 *    still flips. This is the exact construct from a real-world deck whose
 *    slide 12 rendered its photo upside down: the trapezoid flipped correctly,
 *    but so did the photo inside it, when only the outline should have.
 *  - shape 2 ("RotWithShape default", the control): no `@rotWithShape` at all,
 *    so the schema default (`true`) applies and the picture flips WITH the
 *    shape, exactly like before this fix - this shape's `<img>` must carry NO
 *    counter-transform, proving the fix is conditional and does not touch the
 *    overwhelmingly common case.
 *
 * Both shapes reuse a `<p:sp>` (not `<p:pic>`) with `spPr/a:blipFill`, matching
 * how PowerPoint (and this repo's `parseShapeWithImageFill`) represents a
 * shape whose fill IS a picture: this element type parses as `type: "picture"`
 * with the shape's own geometry (`shapeType: "custom"`, `pathData`), not as a
 * `type: "shape"` with a CSS `background-image` fill.
 *
 * Run with: bun run e2e/fixtures/generate-image-fill-rot-with-shape-fixture.ts
 */
...
/** A trapezoid `a:custGeom`, wider at the bottom than the top, in a 400x200 local box. */
const TRAPEZOID_GEOM =
	'<a:custGeom><a:avLst/><a:gdLst/><a:ahLst/><a:cxnLst/><a:rect l="0" t="0" r="0" b="0"/>' +
	'<a:pathLst><a:path w="400" h="200">' +
	'<a:moveTo><a:pt x="100" y="0"/></a:moveTo>' +
	'<a:lnTo><a:pt x="300" y="0"/></a:lnTo>' +
	'<a:lnTo><a:pt x="400" y="200"/></a:lnTo>' +
	'<a:lnTo><a:pt x="0" y="200"/></a:lnTo>' +
	'<a:close/></a:path></a:pathLst></a:custGeom>';

/**
 * A `<p:sp>` trapezoid filled by a picture, `flipV="1"`, with an optional
 * `a:blipFill/@rotWithShape`. `rotWithShape` is omitted entirely (not even
 * written as `"1"`) for the control shape, matching how PowerPoint leaves an
 * unset attribute off rather than writing its own default.
 */
const trapezoidPictureShape = (
	id: number,
	name: string,
	x: number,
	y: number,
	w: number,
	h: number,
	rotWithShape: '0' | undefined,
) =>
	`<p:sp><p:nvSpPr><p:cNvPr id="${id}" name="${name}"/><p:cNvSpPr/><p:nvPr/></p:nvSpPr>` +
	`<p:spPr><a:xfrm flipV="1"><a:off x="${x}" y="${y}"/><a:ext cx="${w}" cy="${h}"/></a:xfrm>` +
	`${TRAPEZOID_GEOM}` +
	`<a:blipFill${rotWithShape ? ` rotWithShape="${rotWithShape}"` : ''}>` +
	`<a:blip r:embed="rId2"/><a:srcRect/><a:stretch><a:fillRect/></a:stretch></a:blipFill>` +
	`<a:ln><a:noFill/></a:ln></p:spPr></p:sp>`;

const shapeRotWithShapeFalse = trapezoidPictureShape(
	2,
	'Trapezoid RotWithShape false',
	40 * EMU_PER_PT,
	40 * EMU_PER_PT,
	280 * EMU_PER_PT,
	140 * EMU_PER_PT,
...
const shapeRotWithShapeDefault = trapezoidPictureShape(
	3,
	'Trapezoid RotWithShape default (control)',
	400 * EMU_PER_PT,
	40 * EMU_PER_PT,
	280 * EMU_PER_PT,
	140 * EMU_PER_PT,
...
zip.file(
	...
	`<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<p:sld xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main"><p:cSld><p:spTree><p:nvGrpSpPr><p:cNvPr id="1" name=""/><p:cNvGrpSpPr/><p:nvPr/></p:nvGrpSpPr><p:grpSpPr/>${shapeRotWithShapeFalse}${shapeRotWithShapeDefault}</p:spTree></p:cSld><p:clrMapOvr><a:masterClrMapping/></p:clrMapOvr></p:sld>`,
);
zip.file(
