/**
 * Generates `image-alpha-effects.pptx`: two pictures for
 * `image-alpha-effects-parity.spec.ts` (regression for GitHub issue #286,
 * "Picture with a:alphaModFix gets the alpha applied twice (SVG filter x CSS
 * opacity)").
 *
 *  - "AlphaModFix Alone" carries only `<a:alphaModFix amt="15000"/>` on its
 *    `a:blip`: a picture dimmed to 15% opacity and nothing else. Before the
 *    fix, `hasAdvancedImageAlphaEffects` treated `alphaModFix` alone as
 *    reason enough to ALSO emit the `imgalpha-<id>` SVG filter (which itself
 *    encoded the same 0.15 multiplier), on top of the identical CSS
 *    `opacity: 0.15` - squaring the alpha to ~0.0225.
 *  - "AlphaModFix Combined" adds `<a:biLevel thresh="50000"/>`, an effect that
 *    genuinely needs the `imgalpha-<id>` SVG filter. This is the "combined
 *    with another alpha/colour effect" case the issue's second commenter
 *    asked to be covered separately: the filter is legitimately built (for
 *    biLevel), but must still not ALSO fold in alphaModFix's own multiplier.
 *
 * See `packages/shared/src/render/image-effects.ts` (`hasAdvancedImageAlphaEffects`,
 * `getImageEffectsOpacity`, `buildImageAlphaFilterMarkup`) for the fix.
 *
 * Run with: bun run e2e/fixtures/generate-image-alpha-effects-fixture.ts
 */
...
/** Picture names the spec locates elements by (authoring order = DOM order). */
export const ALONE_PICTURE_NAME = 'AlphaModFix Alone';
export const COMBINED_PICTURE_NAME = 'AlphaModFix Combined';

/** The alphaModFix amount authored on both pictures, as a 0-1 opacity. */
export const ALPHA_MOD_FIX_OPACITY = 0.15;
...
const picture = (options: {
	...
	`<p:pic><p:nvPicPr><p:cNvPr id="${options.id}" name="${options.name}"/>` +
...
const body = [
	picture({
		id: 2,
		name: ALONE_PICTURE_NAME,
		x: 60,
		y: 60,
		w: 200,
		h: 120,
		blipEffects: '<a:alphaModFix amt="15000"/>',
	}),
	picture({
		id: 3,
		name: COMBINED_PICTURE_NAME,
		x: 320,
		y: 60,
		w: 200,
		h: 120,
		blipEffects: '<a:alphaModFix amt="15000"/><a:biLevel thresh="50000"/>',
	}),
].join('');
...
const out = fileURLToPath(new URL('./image-alpha-effects.pptx', import.meta.url));
