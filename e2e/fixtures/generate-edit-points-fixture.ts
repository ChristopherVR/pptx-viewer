/**
 * Generates `edit-points.pptx` for `edit-points.spec.ts`:
 *
 *  - "EP Star": a plain `star5` preset, the shape Edit Points converts;
 *  - "EP Locked": a rectangle carrying `a:spLocks noEditPoints="1"`, for which
 *    the context menu must grey Edit Points out, as PowerPoint does.
 *
 * The right half of the slide is left empty for the Freeform / Curve tools.
 *
 * Run with: bun run e2e/fixtures/generate-edit-points-fixture.ts
 */
...
const shape = (options: {
	...
	`<p:cNvSpPr>${options.locked ? '<a:spLocks noEditPoints="1"/>' : ''}</p:cNvSpPr><p:nvPr/></p:nvSpPr>` +
...
const body = [
	shape({ id: 2, name: 'EP Star', x: 80, y: 120, w: 240, h: 220, fill: 'DDEBF7', prst: 'star5' }),
	shape({ id: 3, name: 'EP Locked', x: 80, y: 420, w: 240, h: 120, fill: 'F8CBAD', locked: true }),
].join('');
...
const out = fileURLToPath(new URL('./edit-points.pptx', import.meta.url));
