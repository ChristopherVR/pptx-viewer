...
- **3D models** -- Require optional Three.js dependencies (`three`, `@react-three/fiber`, `@react-three/drei`); fall back to poster image without them

### EMF Converter (`emf-converter`)

- **Gradient brushes are simplified** -- `LinearGradient` and `PathGradient` extract only the primary colour rather than rendering full gradient fills
