...
- **Chart editing covers data plus legend** - You can add/remove series, edit data points, add/remove categories, change chart type, and show/hide or reposition the legend (all round-trip on save). The remaining structural properties (axis formatting, data labels, trendlines, error bars) are parsed for display but not yet exposed for programmatic editing.
