...
- **Chart editing covers data, legend, and axis scaling** - You can add/remove series, edit data points, add/remove categories, change chart type, show/hide or reposition the legend, and set axis min/max, major/minor units, number format, and tick-label position (all round-trip on save). The remaining structural properties (axis titles, gridlines and display units, data labels, trendlines, error bars) are parsed for display but not yet exposed for editing.
