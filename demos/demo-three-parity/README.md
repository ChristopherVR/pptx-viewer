...
| 3-D Pie (plain and exploded)                                                           | Done on the fitted camera (slides 14-15): pitch 0.91 x rotX, camera 1.38 box diagonals away, thickness 0.23 x radius, explosion shrinks the radius by 1 / (1 + explosion); rim shading fitted, white seams, hover/select/angular drag. About 3% wider than PowerPoint; exploded cut faces render darker                                                                        |
| Every other 3D chart (bar3D without right-angle axes, line/area with right-angle axes) | Rendered by the pre-element perspective scenes, hosted on the shared renderer (`chart-3d-hosted-stage.ts`) with their interaction intact; not PowerPoint parity                                                                                                                                                                                                                |
...
2. SmartArt follow-ups: Metallic Scene's metal highlight (needs PowerPoint's
   ...
   box, `fontRef` label colours (Metallic's black text), and the pre-existing
   flat-path gaps the harness shows on Basic Pyramid (tiers) and Basic Venn
   (fill transparency).
