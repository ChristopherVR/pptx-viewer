...
| `c:shape` bars without right-angle axes                                      | Done (`render/chart-3d-persp-shape-mesh.ts`): a perspective bar carries its series' or chart's `c:shape` and taper and is drawn with the right-angle chart's cylinder / cone / pyramid solid in its box-space footprint, pointing along the value axis; they used to draw as boxes. Unit tested; no PowerPoint export of the setting in the deck                                                                                                                                                                                                                                                                                                                                                                                              |
...
2. Charts: every slide of the charts deck is on PowerPoint's model, each
   traced corner within ~6pt. Open, and each needs a PowerPoint export the
   parity deck does not have: the surface wireframe's lower layers are an
   approximation, and the settings without an export (bars without
   right-angle axes, including their `c:shape` solids, and line/area/surface
   with right-angle axes) follow the same conventions unverified.
