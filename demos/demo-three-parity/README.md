...
| Every other 3D chart (line, area, pie, surface; bar3D without right-angle axes) | Rendered by the pre-element perspective scenes, now hosted on the shared renderer (`chart-3d-hosted-stage.ts`) with their interaction intact. Same look as before: NOT PowerPoint parity yet                                                                                                                                        |
...
1. Charts: move each perspective chart onto PowerPoint's own model (the
   oblique scene covers slides 1-9): line ribbons, area slabs, pie tilt /
