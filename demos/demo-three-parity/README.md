...
| Every other 3D chart (line/area with right-angle axes)                       | Rendered by the pre-element perspective scenes, hosted on the shared renderer (`chart-3d-hosted-stage.ts`) with their interaction intact; not PowerPoint parity                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
1. Charts: every slide of the charts deck is on PowerPoint's model, each
   ...
   an approximation; line/area with right-angle axes still use the hosted
   scenes (no ground truth in the deck for them).
2. SmartArt follow-ups: whole-slide MAE (0-255, 960x540 against `gt/`,
   Chromium + SwiftShader) for Basic Block List is now flat 5.4, Polished 9.5,
   Inset 11.2, Cartoon 8.5, Brick 7.3, Metallic 6.0, Sunset 7.6, Bird's Eye
   6.4 (was 6.4, 10.4, 12.0, 9.5, 9.6, 13.7, 9.1, 9.1); Basic Pyramid went
   from ~24 to 6-9 and Basic Venn from 11-29 to 3-24. Open: only `threePt`
   has fitted rig lights (other rigs keep the default highlight, and a
   parallel view keeps it too because the bevel constants were fitted with
   it); the bevel bands of a scene style (Metallic's bright top edge, Brick's
   cyan extrusion top) and Inset's groove are approximate; the label font
   falls back when the deck's theme font (Aptos) is not installed, so labels
   run wider than PowerPoint's; Basic Venn's bevel styles (slides 76-84) still
   differ in the overlap shading; Basic Pyramid's labels are black in
   PowerPoint although the cached `fontRef` says `lt1`.
