...
1. Charts: every slide of the charts deck is on PowerPoint's model, each
   ...
   an approximation. Every 3D chart family now draws on the fitted models;
   the settings the deck has no export for (bars without right-angle axes,
   line/area/surface with them) follow the same conventions unverified.
