...
`SlideSizes` has seven entries (EMU values): `WIDESCREEN_16_9`, `WIDESCREEN_16_10`, `STANDARD_4_3`, `A4_LANDSCAPE`, `A4_PORTRAIT`, `LETTER_LANDSCAPE`, and `LETTER_PORTRAIT`. The `theme.colors` object accepts the OOXML scheme slots (`dk1`, `lt1`, `dk2`, `lt2`, `accent1`-`accent6`, …) and `theme.fonts` accepts `majorFont` / `minorFont`.
