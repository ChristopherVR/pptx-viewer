...
- **Export raster des grandes transformations CSS 3D** - Une diapositive portant une grande transformation de perspective CSS explicite est rasterisee un peu moins bien par la voie d'export foreignObject par defaut que par le repli html2canvas (ecart moyen par canal superieur d'environ 11 unites sur la diapositive de test), car Chromium decode le sous-arbre transforme depuis une image SVG avec une qualite moindre ; les formes 3D propres au deck ne sont pas concernees et s'exportent mieux via foreignObject.
