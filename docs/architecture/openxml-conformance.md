...
- The main master carries the deck's own text styles, its placeholder shapes (title, body, date, footer, slide number), a colour scheme derived from its theme, and the `RoundTripTheme12Atom`/`RoundTripColorMapping12Atom`/`RoundTripOArtTextStyles12Atom` records PowerPoint 2007+ writes (`master-roundtrip-writer.ts`), so the reopened `SlideMaster.TextStyles`, theme colours and fonts match the source deck. Group members are written as PowerPoint writes them (`fChild` and an `OfficeArtChildAnchor` in the group's child space), and a compound file past 109 FAT sectors gets DIFAT sectors, so decks with groups, tables or over about 7 MB open in PowerPoint (all COM-verified).

**Remaining `.ppt` gaps:** only the first slide's master is written, so a deck with several masters gets that one on every slide; video and any embedded audio format other than WAV degrade to a picture (see above); and encrypted `.ppt` import supports RC4 CryptoAPI only, not the pre-CryptoAPI Office 95 scheme (also above).
...
Measured with `scripts/gen-smartart-gallery-baseline.ts` (per-fixture numbers in `smartart-gallery/baseline.json`): the 2026-09-24 run the tenth wave below started from, the tenth wave's result, and the eleventh wave's (2026-09-25, later):

| Check                                                          | 2026-09-24 | Tenth wave | Eleventh wave |
| -------------------------------------------------------------- | ---------- | ---------- | ------------- |
| Same set of text-bearing shapes as PowerPoint                  | 228 of 229 | 228 of 229 | 228 of 229    |
| ...and every shape's x/y/w/h within 1% of the diagram's bounds | 97 of 229  | 126 of 229 | 143 of 229    |
| ...within 5%                                                   | 128 of 229 | 161 of 229 | 181 of 229    |
| ...within 10%                                                  | 137 of 229 | 171 of 229 | 191 of 229    |
| ...within 50%                                                  | 194 of 229 | 217 of 229 | 226 of 229    |
| Same shape preset on every matched shape                       | 212 of 229 | 222 of 229 | 224 of 229    |
| Same font size on every matched shape                          | 24 of 229  | 114 of 229 | 129 of 229    |
| Full gate (all of the above at 1%)                             | 11 of 229  | 97 of 229  | 113 of 229    |

By gallery category, the share within 1% is now: timeline 7/7, matrix 3/3, Meet the Team 5/6, pyramid 7/10, Text Card 9/13, list 28/40, cycle 11/17, process 31/48, hierarchy 14/23, relationship 19/35, picture 9/27. The three fixtures still beyond 50% are Arrow Ribbon, Balance and Varying Width List.

One of the three structural misses predates this wave: `bubble-picture-list--hier5` (a preset-specific blank separator paragraph); `segmented-process--hier5` (a `linDir="fromB"` stack whose items switch between two composite templates by `revPos`) was ALSO a pre-existing miss but the ninth wave below moved it onto `engine-first-allowlist.ts` with a perfect (0 deviation) match, closing it. The other two, `numbered-title-list--hier5` and `numbered-card-list--hier5`, are a side effect of the eighth wave below: fixing their real shape-drop bug moved them onto `engine-first-allowlist.ts`, but the engine still ALSO renders each accented item's decorative background shape with its own (redundant, unfolded) copy of the item's self text, alongside the correct folded self+descendant text on the actual text overlay - two extra, harmless-but-inexact shapes per fixture, tracked as an open gap in the eighth wave's own paragraph rather than left undocumented. By gallery category, the share within 1% is: matrix 3/3, pyramid 7/10, cycle 10/17, hierarchy 11/23, relationship 15/35, list 15/40, process 19/48, picture 7/27, and 0 for timeline (7), Meet the Team (6) and Text Card (13) (pre-ninth-wave category shares; see the ninth wave below for what actually moved).
...
Sixth, the engine gained a `pyra` algorithm (`smartart-engine/alg-pyra.ts`, ECMA-376 Part 1 21.4.2.x: stacks the data points as equal-height horizontal bands, each band's width proportional to its position in the stack, `linDir="fromT"` pointing the apex down for "Inverted Pyramid"), porting the band-stacking geometry from the legacy `arrangePyramid`. A second unit bug surfaced once real bands rendered: real "Basic Pyramid" also declares its "level" band's OWN self-scoped `<dgm:constr type="h" val="500"/>`/`<dgm:constr type="w" val="1"/>`, and `preferred-size.ts`'s `preferredSize` (by design, to preserve a genuine self-declared aspect ratio) re-applies a composite child's own self-scoped size constraints on top of whatever its parent already assigned it - overriding the fifth wave's fix and re-collapsing "level" (measured: engine deviation 1.6079, still declining on every dataset). No generic rule can tell "level"'s declaration apart from `radial-cycle`/`segmented-cycle`/`vertical-chevron-list`'s own `dummyConnPt`/`wedge*` nodes, which declare the SAME bare self `w val="1"`/`h val="1"` shape and MUST keep it as a real ~2.83pt marker (their own position comes from a sibling `for="ch"` declaration, exactly like "level"'s), so `alg-pyra.ts` sanitizes only the specific named node its own `pyraLvlNode` param points at (`"level"` by default), dropping its bare-literal self `w`/`h` before the rest of the tree lays out. That fix is scoped to the `pyra` algorithm's own per-item subtree and cannot reach `dummyConnPt`/`wedge*` (different algorithms, different layoutDefs entirely). With both fixes, Basic Pyramid's and Inverted Pyramid's `flat3` (no accented rows) datasets now match legacy EXACTLY (0.0019, tied); `hier5`/`hier8` (accented rows) still measure 0.5663, since `pyraAcctRatio`/`pyraAcctPos` accent-column splitting (`repositionPyramidBands` in the legacy interpreter) is a genuinely separate, per-item-role, post-composite geometry pass not ported in this wave - a tracked, documented limitation, not a silent regression. Because the strict allowlist rule requires improvement (or a tie) on EVERY dataset of a layout with no shape-set loss, and `hier5`/`hier8` are measurably worse than legacy (0.5663 vs. 0.0019), neither Basic nor Inverted Pyramid newly qualifies for `engine-first-allowlist.ts` this wave. Re-measuring the full 229-fixture corpus confirms the `pyra` algorithm change touches ONLY these two layouts (`measure-smartart-engine-vs-legacy.ts` diffs identically save those two rows) with zero regressions and the gate numbers unchanged (still 87/229 geometry within 1%, 11/229 full gate; `pyra` is not yet allowlisted, so production behaviour - legacy-first for these two layouts - is unchanged either).

Seventh, the `pyraAcctRatio`/accent-column gap the sixth wave left open was closed: `alg-pyra.ts` gained a POST-pass (`applyPyraAccentSplit`, run by `layout-driver.ts`'s new `LayoutRegistry.resolvePost` hook after a `pyra` node's children have finished laying out their own subtrees), porting the legacy interpreter's COM-verified `repositionPyramidBands`. Once `pyraAcctRatio` resolves positive (real "Basic Pyramid"/"Inverted Pyramid", `dgm:choose`-gated on `func="maxDepth" op="gte" val="2"`), every row's own "level" band shrinks to `(1 - pyraAcctRatio)` of its natural width, anchored at the diagram box's own left edge, and a row whose data point has a child gets an "acctBkgd"/"acctTx" column filling the remainder out to the diagram's right edge. The first attempt filled that remainder from "level"'s own shrunk (WIDE-edge) right corner and measured 0.1142 on `hier5`/`hier8` (down from the sixth wave's 0.5663, but still worse than legacy's 0.0019); comparing per-shape geometry against the cached ground truth found the accent column's true left edge is the row's scaled NARROW (top) trapezoid corner, not the wide one `arrangePyra`'s own tight bounding box collapsed to, so `applyPyraAccentSplit` recomputes that corner independently (mirroring the legacy interpreter's own `pyramidRowGeometry`). With that correction, Basic Pyramid's and Inverted Pyramid's `hier5`/`hier8` datasets tie legacy exactly (0.0019, matching `flat3`), so both layouts now qualify for `engine-first-allowlist.ts` under the tie-or-better rule and route engine-first in production. Re-measuring the full 229-fixture corpus confirms zero regressions elsewhere (`measure-smartart-engine-vs-legacy.ts` and `gen-smartart-gallery-baseline.ts --compare` unchanged outside the two pyramid rows) and the gate numbers hold at 87/229 geometry within 1%, 11/229 full gate (both pyramid layouts already matched within 1% via legacy before this wave, so the routing change alone does not move those counts).

Eighth, `numbered-title-list--hier5.pptx`/`numbered-card-list--hier5.pptx` (real "Numbered Title List"/"Numbered Card List") dropped 3 of their 6 cached shapes: the "1"/"2"/"3" ordinal badges rendered blank. Their layoutDef's `sibTransNodeCircle` presents its badge text via `dgm:presOf axis="self" ptType="sibTrans"` - the connector text lives on the SIBLING transition point, not the PARENT one (`smartart-node-tree-axis.ts`'s own doc comment already documents this asymmetry for the legacy interpreter: "`numbered-dots-horizontal--hier5.pptx`'s "1"/"2"/"3" ordinal badges live on the `sibTrans` point; its paired `parTrans` is always blank"). `data-points.ts`'s `buildDataModel` copied a connection's resolved `label` onto its synthesised `parTrans` `DataPoint` but never onto the paired `sibTrans` one, and `engine-to-result.ts` only ever read a presented point's content-node `.text` (via `.source`), never a transition point's own `.label` - so a badge presented off the sibling side had no text to find on either end of that chain. Fixed both: `sibTrans` now carries `label` like `parTrans` already did, and `buildRenderedNode` falls back to a presented point's `.label` (exposed as `RenderedNodeIdentity.literalText`, the SAME field the drawing bridge already prioritises for a transition-point role) when no content point is presented. With badges no longer blank, both layouts now match all 6 cached shapes (previously 3 of 6) and measurably beat legacy on their one dataset each (Numbered Title List 0.5854 -> 0.3415, Numbered Card List 0.4465 -> 0.2458), qualifying for `engine-first-allowlist.ts`. This is a general per-point engine fix, not scoped to numbered lists specifically, so re-measuring the full corpus (as the allowlist's own doc comment requires after any engine change) found three more, unrelated layouts newly qualifying from the same fix: Converging Arrows and Diverging Arrows (legacy 0.0638 -> engine 0.0375, an opposing-arrow pair's own middle "VS"-style sibTrans label) and Tabbed Arc (legacy 0.2364 -> engine 0.0131) - all three match every cached shape with no extras, unlike the two numbered-list layouts. The two numbered-list fixtures ARE the source of this wave's four (up from two) structural misses: each accented item's `bgRect` (a real, undocumented-as-such background shape, `dgm:presOf axis="self"`, no `hideGeom`) independently presents the same "self" content point the item's actual text overlay (`nodeText`, `hideGeom="1"`, `dgm:presOf axis="desOrSelf"`) also presents, so on a row WITHOUT a descendant both render identical text and collide harmlessly under the gate's own exact-text dedup, but on a row WITH one, `bgRect`'s unfolded self-only text ("Node One") and `nodeText`'s folded self+descendant text ("Node One\nNode Two has a longer label") are different strings and both survive as separate, unmatched shapes. Root-causing that fully (when should a background sibling's OWN `presOf` be suppressed in favour of a `hideGeom` overlay's, matching real PowerPoint's actual single-text-per-item rendering) is a genuinely separate, deeper question about presentation-node precedence, not a shape-loss bug like the badge fix above; the layoutDef's own `moveWith="bgRect"` attribute on `nodeText` (already parsed into `LdLayoutNode.moveWith` but never threaded through `PresNode`/`EngineNode`) is the likely ECMA-376 signal such a fix would key off, tracked here as an open, documented gap rather than attempted in this wave. Re-measuring the full 229-fixture corpus with `gen-smartart-gallery-baseline.ts --compare` confirms geometry-within-1% and the full gate hold exactly (still 87/229, 11/229 - no regression on either), while structural drops 227->225 (the two numbered-list phantom-shape fixtures above) and within-5%/within-10%/within-50% each improve slightly (110->113, 121->122, 180->181) from the arrow/Tabbed-Arc wins.

Ninth, attempting `hierRoot`/`hierChild` (ECMA-376 Part 1 21.4.2.6/21.4.2.7, the org-chart/hierarchy family) surfaced a much bigger, unrelated bug: `engine-to-result.ts`'s `isFiniteGeometry` gate declined the WHOLE diagram whenever ANY rendered node had a zero-area box, with no exception for a shape that legitimately renders that way and carries no text (a `conn`-alg node's own `arrange` only produces a real box for its supported 2-D-straight case, leaving `{w:0, h:0}` for `connRout="bend"` routing; a `hierChild` continuation for a childless leaf is deliberately `{w:0, h:0}` too, since it has nothing to fan). Since `SmartArtLayoutResult.connectors` is separately, and always, discarded downstream (`smartart-interpreter-drawing-bridge.ts`'s own doc comment: PowerPoint reconstructs `dsp:cxn` shapes itself), a `conn`-alg node was never meant to reach the rect-shape collector at all, and any node with zero area and no text was never going to render anything visible either way; both are now excluded (`conn`-alg nodes are skipped entirely in `collectRenderedNodes`; `isRenderable` also now rejects a non-positive-area box outright) instead of failing the whole result. This one fix, unrelated to any specific layout family, unblocked 40 previously fully-declining layouts across the corpus at once; re-measuring per the allowlist's own "regenerate after any engine change" rule found 25 of them measurably beat legacy on every dataset with no shape loss, including the entire "Meet the Team" family (previously 4979%-9979% legacy deviation, the worst in the whole corpus, down to under 83% engine deviation) and `segmented-process--hier5` (0.714 -> 0, closing one of the two pre-existing structural misses named above). All 25 are now on `engine-first-allowlist.ts` (99 layouts total). Re-measuring the full 229-fixture corpus with `gen-smartart-gallery-baseline.ts --compare` confirms the gate itself holds (11/229, unchanged) while geometry-within-1% jumps 87->97, within-5% 113->128, within-10% 122->137, within-50% 181->194, preset 204->210, and structural 225->226 (net of the numbered-list phantom-shape cost above and the segmented-process win); font-match count drops 25->24 (one single newly-engine-first fixture's font size is very slightly off where legacy happened to be exact - not chased further, since the allowlist's own inclusion rule does not require font parity and the geometry win is decisive). `hierRoot`/`hierChild` themselves were then actually ported (`smartart-engine/alg-hier.ts`): a real layoutDef hand-unrolls the recursion per generation (`hierChild1` fans the top-level points into `hierRoot1` items, each nesting a `composite` content box and its own nested `hierChild2` continuation one generation deeper, and so on), and every generation must render its item at the exact SAME size - computing that per generation independently (from each `hierChild` call's own local box) gives the right topology but the WRONG scale, so the outermost `hierChild` call instead walks its whole subtree once (leaf-weighted width units, generation depth) to derive ONE shared item size and seeds it onto every descendant before any of them are positioned. Only the plain "std" fanned-row case is implemented (`hierBranch` hanging columns, `chMax`/`chPref` wrapping, and `orgChart` assistant slots - each a genuinely separate, per-item-role branch in the legacy interpreter's own ~8800-line `smartart-hierarchy-*.ts` family - are not attempted). Measured against the whole hierarchy family (8 layouts, 14 fixtures): a substantial improvement over the prior full decline (e.g. real "Hierarchy" 0.7767 -> 0.2687, "Horizontal Hierarchy" 1.0 -> 0.4925), but still measurably worse than the legacy interpreter's own, far more mature implementation on every single dataset (legacy sits at 0.0019-0.3 for this family); `hierRoot`/`hierChild` are therefore NOT added to `engine-first-allowlist.ts` this wave, an honest "attempted, not yet sufficient" outcome matching the fourth wave's own cycle effort. Zero regressions either way, confirmed by the same full-corpus re-measurement above.

Tenth (2026-09-25), four changes, each re-measured on the whole corpus with zero fixtures leaving the 1% band (`gen-smartart-gallery-baseline.ts --compare`):

- **Font size.** The engine fitted every text node alone with a one-line 0.6em guess and an 18pt default. It now starts each `tx` node at its `primFontSz` constraint and shrinks in whole points down to its `dgm:rule` floor (no rule: no shrink) until the text fits the preset's own text rectangle net of its margins; `op="equ"` groups and every node sharing a layout-node name (510 of 514 cached presName groups render at one size) take their worst fit, and a `primFontSz lte` link caps a node only when the referenced node shrank. Measurement itself was corrected against PowerPoint over COM: the Aptos advance table had differenced an AutoSize box's `Shape.Width` and read about 1.1% wide with no kerning, so `scripts/make-font-advance-table.ps1` now measures `TextRange.BoundWidth` plus every pair kerning (applied at 12pt and above, reproducing COM widths of whole labels within 0.2pt); SmartArt applies its 90% line spacing to the font's own 1.2207em line height (1.099-1.100 x size per line on five fixtures), not a text box's 1.2em, plus a fixed 0.00695em per block; margins are points (a length-referenced one is that length in millimetres); an undeclared margin side defaults to 0.56 x size; `stBulletLvl` decides which paragraphs are bullets, which render at `secFontSz` (0.78 x). On the cached geometry alone, 941 of 950 cached text shapes now fit at their cached size under this model (900 before). The legacy interpreter shares the metric fixes and its tiered fit now walks whole points.
- **Hierarchies.** `alg-hier.ts` now measures the whole tree once in the definition's unscaled space, packs sibling subtrees by outline rather than bounding box (a hanging child juts past its parent without widening the row, and `smartart-orgchart-nested-hang.pptx`'s teams sit under their parent's leaf siblings), places each node on its `hierAlign` side (a `tL`/`tR` node over `fromT` columns hangs them `0.25 W` in, the org charts' spelled-out `init`/`l`/`r`/`hang` branches), then scales once to fit and centres on what is drawn. Organization Chart (0.0231 -> 0.0019), Horizontal Organization Chart (0.0225 -> 0.0019) and Horizontal Multi-Level Hierarchy (0.2987 -> 0.0012) route engine-first, as do Hierarchy and Horizontal Hierarchy (a 0.0019 geometry tie broken by every font matching). Diagrams with assistant points still go to the legacy interpreter.
- **Snake grids.** `alg-snake.ts` now keeps each cell at its constraint size, uses the `sibTrans` spacer's width and the node's `sp` as the gaps, and picks the line length whose grid scales largest (shrinking, never growing): Basic Block List, the Bending Process and Bending Picture families, Picture Grid and the Text Card family move to 0-0.0225.
- **Shapes.** A `dgm:shape` with no `type` no longer draws a phantom rounded rectangle, and a borderless box whose first presented point is an empty placeholder still draws (the dot-list layouts).

`scripts/measure-smartart-engine-vs-legacy.ts` now also breaks a geometry tie on font matches (Basic Cycle, Titled Matrix), and `engine-first-allowlist.ts` lists 131 layouts (99 before).

What does hold: the core single-algorithm layouts (Basic Process, Vertical Process, Basic Block List, Basic/Continuous/Multidirectional Cycle, Basic and Diverging Radial, Hierarchy, Horizontal Hierarchy, Basic and Inverted Pyramid, Organization Chart, and since the tenth wave the snake grids and text cards above) reproduce PowerPoint's geometry within 1% on every dataset in the corpus, and 129 of 229 fixtures match every cached font size (after the eleventh wave below). Text autofit follows the measured rules (whole-point sizes, the real frame margins and rounded-corner insets, a fixed 0.78 ratio for folded child paragraphs), and org charts are additionally pinned by `smartart-orgchart-genuine-fixture.test.ts` (topology, hanging-tail offset, fan-vs-column choice).

Eleventh (2026-09-25, later), three changes, each re-measured with zero fixtures leaving the 1% band (`gen-smartart-gallery-baseline.ts --compare`) and the allowlist regenerated (131 -> 143 layouts):

- **Text-driven sizing.** A node whose geometry depends on its text's font size (a child `h` of `fact x primFontSz`, a `tx` node whose `dgm:rule type="h" val="INF"` lets it grow to its text) no longer collapses: the node owning the `primFontSz` rule searches whole points down from its constraint until every text fits its (grown) box and nothing spills out, with font-relative lengths resolved at the candidate size, growth bounded by the node's own `lte` constraints and equalised across `op="equ"` groups. A grown box is its measured text plus margins inside the preset's text rectangle plus 0.04em (cached single lines at 30, 35 and 46pt: 0.043, 0.040, 0.040em). Vertical Bullet List, Vertical Box List and Horizontal Bullet List move to 0.0019, Basic Chevron Process to 0.0075 and Sub-Step Process to 0.0281, all engine-first.
- **Org-chart assistants.** Assistants sit in a band one `sp` below the manager and above every other row, paired either side of the manager's centre line (the left one first, `sibSp / 2` from the line), further pairs one `sp` lower; the same two-column placement lays out a "Both Hanging" manager's reports, a hanging column's inset reads the manager's `alignOff` (0.65 with an assistant), and each manager's own `hierBranch` is read from its presentation point. All 21 corpus org charts with assistants (1 to 4 assistants, all four `hierBranch` values, `smartart-orgchart-assistants.pptx` authored over COM for this) are within 0.0017 with every font size matched (legacy 0.22-0.47); Half Circle Organization Chart moves to 0.0012.
- **Rule-shrunk grids and runs.** A snake whose cells a `dgm:rule type="w"` shrinks while the cells grow to their content is re-laid per candidate width: PowerPoint bisects between the rule's `val` and the constrained width with five probes, keeping the last that fits (EMU-exact on Meet the Team flat3 and hier5). A lin run its flow reference cannot shorten shrinks both reference axes, and a nested lin sized by nothing across the flow is as thick as its content. Meet the Team 0/6 -> 5/6 within 1%, Icon Circle Label List 0.4152 -> 0.0012, Process List 0.8762 -> 0.0012, Vertical Equation 0.1745 -> 0.0012.

Still open: Meet the Team hier8 (0.0496: a card whose role text grows is not modelled inside the probes), the name lines' bold weight (no Aptos Bold metrics, so the Meet the Team name font is one point large), Name and Title Organization Chart (0.0415), the labelled and table hierarchies, Circle Picture Hierarchy, horizontal org charts with assistants (still legacy), and three fixtures beyond 50% (Arrow Ribbon, Balance, Varying Width List). This only affects decks saved without the cached drawing; PowerPoint itself writes one, so it applies to decks from other generators and to live re-layout after a structural edit.
