...
| SmartArt-Layout | Angenähert ohne zwischengespeicherte Zeichnung | Präsentationen, die ohne die zwischengespeicherte `dsp:drawing` gespeichert wurden, werden von einer DiagramML-Engine pro Datenpunkt (131 Layouts) oder vom älteren Familien-Interpreter angeordnet. Von 229 per COM erstellten Galerie-Fixtures erzeugen 228 dieselbe Formenmenge wie PowerPoint, 126 treffen dessen Geometrie auf 1 % genau (161 auf 5 %) und 114 jede Schriftgröße. Layouts, deren Felder mit ihrem Text wachsen (Vertical Bullet List, Vertical Box List), Assistenten in Organigrammen und die Meet-the-Team-Karten bleiben ungenau. Siehe [OpenXML-Konformität](/architecture/openxml-conformance#smartart-layout-ground-truth) für die Messbelege.                        |
...
- **Manche Filterfamilien und Voreinstellungen werden bei der Wiedergabe angenähert:** `strips` spielt als Kantenwischblende ab, `wedge` als wachsendes Sechseck, `slide`/`cover`/`uncover`/`push`/`pull` teilen sich ein Einfliegen, und 45 PowerPoint-Voreinstellungs-IDs spielen einen Ersatzeffekt ab (zum Beispiel werden Einfaches Drehen und Hinausschweben als Überblendung abgespielt).
...
| WordArt-Hüllenverformungen (`inflate` / `deflate` / `can` / `slant` / `fade` / `cascade` / ...) | Can-Innenfläche und Schriften ohne Datei unverifiziert | Die Can-Voreinstellungen platzieren Glyphen jetzt nach PowerPoints gemessenem linearem Abstandsgesetz (per COM ermittelt, unabhängig vom Anpassungswert der Kurve). Noch offen: die erneute Messung des verbleibenden Umrissfehlers im Inneren der Can-Voreinstellungen gegen PowerPoint sowie die Validierung des nachgezeichneten Umrisspfads gegen PowerPoint, der für Schriften verwendet wird, deren Datei nicht erhältlich ist. Siehe [Visuelle Effekttreue](/guide/visual-effects) für die Herkunft.                         |
...
- **Das Speichern einer bearbeiteten Folie kann noch kleinere Markup-Details verändern.** Formeln, Zeilenumbrüche, geerbte Formatierung, Master-Textformate, Design-Hintergründe, Kommentar-Zeitstempel, Bildfüllungen, Medien-Klickaktionen, Run-Sprachen, Run-Eigenschaften, Innenschattenfarben, Verlaufseinzüge und unveränderte Diagramme überstehen jetzt den Round-Trip; ebenso Aufzählungszeichenfarben bei geerbten Aufzählungszeichen, eine explizit festgelegte Standard-Tabulatorausrichtung, Metadaten von Animationen und folienübergreifend abgespieltem Audio sowie eine unbenutzte Liste von Kommentarautoren. Auf einer neu geschriebenen Folie bleibt ein kleiner Rest: Einige Run-Attribute (`err`, `b`) und Eigenschaften von Ruby-Runs werden explizit ausgeschrieben, manche Formen erhalten eine explizite Umrissbreite, und Revision, Änderungszeitpunkt und Folienanzahl in `docProps` werden aktualisiert. Unbearbeitete Folien überstehen den Round-Trip unverändert.
- **Text:** CJK-Satzzeichen, die PowerPoint über den rechten Rand hinausragen lässt (`hangingPunct`), werden stattdessen in die nächste Zeile umbrochen, weil nur Safari hängende Satzzeichen in CSS umsetzt, und `eaLnBrk="0"` schaltet die ostasiatischen Zeilenumbruchregeln (Kinsoku) nicht so ab wie PowerPoint.
- **Diagramme:** Legendenfelder und Führungslinien von Datenbeschriftungen werden nur für Balken- und Säulendiagramme gezeichnet.
