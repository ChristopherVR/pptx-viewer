...
| SmartArt-Layout | Naeherung ohne zwischengespeicherte Zeichnung | Praesentationen, die ohne die zwischengespeicherte `dsp:drawing` gespeichert wurden, ordnet eine DiagramML-Engine pro Datenpunkt (131 Layouts) oder der aeltere Familien-Interpreter an. Von 229 per COM erstellten Galerie-Fixtures erzeugen 228 PowerPoints Formenmenge, 126 treffen dessen Geometrie auf 1 % genau (161 auf 5 %) und 114 jede Schriftgroesse. Layouts, deren Felder mit dem Text wachsen (vertikale Aufzaehlungs- und Feldliste), Assistenten in Organigrammen und die "Meet the Team"-Karten bleiben ungenau. Siehe [OpenXML-Konformitaet](/architecture/openxml-conformance#smartart-layout-ground-truth) fuer die Messbelege.            |
...
- **Noch nicht unterstuetzt:** buchstabenweises Kraeuseln innerhalb eines absatzweisen Aufbaus, sowie die Erstellung der p15-Uebergaenge (sie spielen ab, wenn sie in einer Datei vorhanden sind, aber ihre Richtungsoptionen werden ignoriert); ein `p14:bounceEnd` von 100 % (keine Bewegung mehr, von PowerPoint selbst unstet dargestellt) wird auf 95 % begrenzt. Ausloeser auf einer Medien-Textmarke ("Bei Textmarke") lassen sich in allen fuenf Bindings erstellen, und die Ausschwingkurve von "Abprallen am Ende" ist an PowerPoints eigene Frames angepasst.
...
Ein Audit vom September 2026 gegen echtes PowerPoint fand diese noch offenen Luecken:

- **Das Speichern einer bearbeiteten Folie kann noch kleinere Markup-Details veraendern.** Formeln, Zeilenumbrueche, geerbte Formatierung, Master-Textformate, Theme-Hintergruende, Kommentar-Zeitstempel, Bildfuellungen, Medien-Klickaktionen, Laufsprachen, Laufeigenschaften, Innenschatten-Farben, Verlaufs-Einzuege und unveraenderte Diagramme werden jetzt sauber gespeichert; ein kleiner Rest seltener Attribute (etwa manche `buClr`, `tabLst@algn` und Animations-/Audio-Metadaten) kann auf einer neu geschriebenen Folie noch abweichen. Unveraenderte Folien werden sauber hin- und zurueckkonvertiert.
- **Text:** CJK-Satzzeichen, die PowerPoint ueber den rechten Rand haengen laesst (`hangingPunct`), werden stattdessen in die naechste Zeile umbrochen, weil nur Safari haengende Satzzeichen in CSS umsetzt, und `eaLnBrk="0"` schaltet die ostasiatischen Zeilenumbruchregeln (Kinsoku) nicht wie in PowerPoint ab.
- **Diagramme:** Legenden-Sprechblasen und Fuehrungslinien von Datenbeschriftungen werden nur fuer Balken- und Saeulendiagramme gezeichnet.
- **Animationen und Uebergaenge:** die Richtung des Zoom-Uebergangs, das Erstellen eines Triggers auf einer Medien-Textmarke und die genaue Form der `p14:bounceEnd`-Ausklingkurve entsprechen noch nicht PowerPoint.
- **3D-Modelle** ignorieren die in PowerPoint erstellte Kamera, Transformation und Beleuchtung.
- **Editor-Abdeckung** ist eine Teilmenge der von PowerPoint: mehrere Ribbon-Galerien sind noch nicht verfuegbar. Punkte bearbeiten (mit den Zeichenwerkzeugen Freihandform: Form und Kurve), Formen zusammenfuehren, das Zuschneiden von Bildern auf der Zeichenflaeche (Zuschneide-Griffe, Auf Seitenverhaeltnis zuschneiden, Ausfuellen, Anpassen), Inhalte einfuegen (Spezial), die Kontextmenues fuer Zeichenflaeche und Elemente, die Mehrfachauswahl im Folienbereich, die echte Animationsvorschau und die Standard-Tastenkuerzel sind in allen fuenf Bindings verfuegbar.
