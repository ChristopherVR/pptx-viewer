...
- **Raster-Export grosser CSS-3D-Transformationen** - Eine Folie mit einer grossen expliziten CSS-Perspektivtransformation wird ueber den Standard-Export per foreignObject etwas schlechter gerastert als ueber den html2canvas-Fallback (mittlere Kanalabweichung etwa 11 Einheiten hoeher auf der Testfolie), weil Chromium den transformierten Teilbaum aus einem SVG-Bild mit geringerer Qualitaet dekodiert; die eigenen 3D-Formen des Decks sind nicht betroffen und exportieren ueber foreignObject besser.
