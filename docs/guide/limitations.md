...
- **Chart editing covers data and most formatting** - You can add/remove series and categories, edit data points, and change chart type and grouping (clustered/stacked/percentStacked). Beyond the data itself, the inspector now also edits axis formatting (min/max, major/minor units, display units, number-format code, axis titles, tick-label position, major/minor gridline toggles), legend visibility and position, chart-level data labels (value/category/series-name/percent/legend-key, position), and per-series trendlines and error bars. Not yet exposed for editing: series colour/fill and markers, per-data-point formatting overrides, logarithmic axis scaling, gridline/axis-title styling, and per-series chart type within combo charts.
