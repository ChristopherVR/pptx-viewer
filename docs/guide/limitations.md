...
- **Strict OOXML conformance round-trips fully** - Office 365 can save files in ISO/IEC 29500 Strict mode, which uses different namespace URIs than the common Transitional (ECMA-376) format. The engine normalises Strict to Transitional on load and converts back on save. Rather than depending on a fixed lookup table, it applies the structural Strict/Transitional rule (host swap plus the `2006` version segment) across the PresentationML, DrawingML, SpreadsheetML, WordprocessingML, and officeDocument namespace families, so every namespace in those families round-trips, including strict-only relationship types and sub-namespaces that are not individually enumerated. Open Packaging Conventions and Markup Compatibility namespaces (which are conformance-independent) and third-party extension namespaces (for example Microsoft's) are preserved unchanged.
...
- **Vue / Angular feature parity** - All three bindings share the same rendering engine and render slides identically. SmartArt editing (node add/remove/reorder/promote/demote, text editing, layout switching, and colour-scheme/style selection through the inspector) is now available in React, Vue, and Angular. The remaining React-only features being ported incrementally are presenter mode, the export dialogs, parts of the inspector panel, find/replace, and real-time collaboration.
