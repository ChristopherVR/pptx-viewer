...
| Chart direct manipulation | Varies by chart kind, and by binding | Bar, line, area, scatter, and bubble marks can be dragged to new values on the canvas (click a mark to select, double-click the title to rename). Pie, doughnut, radar, and stacked/percent-stacked marks are click-to-select with values edited in the inspector. Map and 3D surface charts render as static SVG. **Dragging is wired in the React, Vue and Angular renderers only**: the Svelte and vanilla renderers tag the marks but do not yet handle the drag, so use the inspector there. |
