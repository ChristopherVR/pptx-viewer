...
| Feature           | Status                 | Notes                                                                                                                                                                                                                                                                                                                     |
| ----------------- | ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gradient brushes  | Exact stops and tiling | GDI+ linear and radial gradients render exact colour stops, presets, blend factors and transforms. Since emf-converter 3.1.0, `WrapMode` tiling works at any angle and path gradients follow their boundary shape (within 1-6% of Windows GDI+ depending on wrap mode). Texture (image) brushes still render solid black. |
| Raster operations | ROP3 exact             | Since 3.1.0, all 256 ROP3 codes are evaluated exactly for `BitBlt`/`StretchBlt`/`StretchDIBits`. Bitwise ROP2 pen modes (AND/OR/XOR) are approximated.                                                                                                                                                                    |
| Text              | Browser font engine    | Glyph metrics can differ from Windows GDI. Since 3.1.0, `ExtTextOut` `dx` arrays, the `LOGFONT` height sign and escapement are honoured; without a `dx` array, spacing depends on the browser's font substitution. Rotated or sheared world transforms in plain GDI (non-GDI+) metafiles are not applied.                 |
