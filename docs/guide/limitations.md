...
- **Editor coverage** is a subset of PowerPoint's: Edit Points and several ribbon galleries are not available yet. Merge Shapes, on-canvas picture cropping (crop handles, Crop to Aspect Ratio, Fill, Fit), Paste Special, the empty-canvas and element context menus, slides-pane multi-select, real in-place animation preview and the standard editing shortcuts are available in all five bindings.
