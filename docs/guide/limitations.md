...
| Transition authoring | Every modelled type round-trips          | All 58 transition types, including the Office 2010+/2013+/2016 extension effects, are authored into the markup PowerPoint reads (an `mc:AlternateContent` Choice with a `p:fade` fallback). The remaining gaps are listed under [Transition authoring](#transition-authoring) below. |
...
| Preset coverage in playback                                                                                                               | 39 of ~150 preset IDs | Unmapped presets fall back to a generic fade/pulse that preserves show/hide semantics rather than being dropped. The saved file keeps the real preset ID, so PowerPoint plays the right effect. |
...
| Area                                            | Status                      | Notes                                                                                                                                                                                                                     |
| ----------------------------------------------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Cube / Rotate / Orbit                           | Authored, reload as `prism` | PowerPoint has no element of its own for these: it writes all of Cube, Box, Rotate and Orbit as `p14:prism` with two flags. They save and replay correctly, but a reload types them as the generic `prism`.               |
