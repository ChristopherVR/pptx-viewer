...
| Diseño de SmartArt   | Aproximado sin dibujo en caché | Las presentaciones guardadas sin el `dsp:drawing` en caché se maquetan con un motor DiagramML por punto (131 diseños) o con el intérprete por familias más antiguo. Frente a 229 fixtures de la galería creados mediante COM, 228 producen el mismo conjunto de formas que PowerPoint, 126 coinciden con su geometría con un margen del 1 % (161 con un margen del 5 %) y 114 coinciden en todos los tamaños de fuente. Siguen siendo inexactos los diseños cuyos cuadros crecen con su texto (Vertical Bullet List, Vertical Box List), los asistentes de los organigramas y las tarjetas de Meet the Team. Consulte [Conformidad OpenXML](/architecture/openxml-conformance#smartart-layout-ground-truth) para ver las mediciones. |
...
- **Algunas familias de filtros y algunos preajustes se aproximan al reproducirse:** `strips` se reproduce como un barrido desde el borde, `wedge` como un hexágono que crece, `slide`/`cover`/`uncover`/`push`/`pull` comparten un mismo desplazamiento hacia dentro, y 45 ID de preajustes de PowerPoint reproducen un efecto sustituto (por ejemplo, Giro básico y Flotar hacia fuera se reproducen como un desvanecimiento).
...
| Deformaciones de envolvente de WordArt (`inflate` / `deflate` / `can` / `slant` / `fade` / `cascade` / ...) | Interior de can y fuentes sin archivo sin verificar | Los preajustes can colocan ahora los glifos según la ley de espaciado lineal medida en PowerPoint (obtenida mediante COM, independiente del valor de ajuste de la curva). Queda pendiente volver a medir frente a PowerPoint el error restante del contorno interior de los preajustes can, y validar frente a PowerPoint la ruta de contorno trazado que se usa para fuentes cuyo archivo no se puede obtener. Consulte [Fidelidad de efectos visuales](/guide/visual-effects) para ver su procedencia.                                                   |
...
- **Guardar una diapositiva editada todavía puede alterar detalles menores del marcado.** Las ecuaciones, los saltos de línea, el formato heredado, los estilos de texto del patrón, los fondos del tema, las marcas de tiempo de los comentarios, los rellenos de imagen, las acciones de clic en elementos multimedia, los idiomas y las propiedades de los fragmentos de texto, los colores de las sombras interiores, las sangrías de los degradados y los gráficos sin modificar ya se conservan en el ciclo de ida y vuelta; también los colores de viñeta sobre una viñeta heredada, la alineación de tabulación predeterminada definida explícitamente, los metadatos de animación y de audio que se reproduce entre diapositivas, y una lista de autores de comentarios sin usar. En una diapositiva reescrita queda un pequeño residuo: algunos atributos de fragmento (`err`, `b`) y las propiedades de los fragmentos ruby se escriben explícitamente, algunas formas reciben un ancho de contorno explícito, y en `docProps` se actualizan la revisión, la hora de modificación y el recuento de diapositivas. Las diapositivas no editadas se conservan intactas.
- **Texto:** la puntuación CJK que PowerPoint deja colgar más allá del margen derecho (`hangingPunct`) pasa a la línea siguiente, porque solo Safari implementa la puntuación colgante de CSS, y `eaLnBrk="0"` no desactiva las reglas de salto de línea de Asia oriental (kinsoku) como lo hace PowerPoint.
- **Gráficos:** los cuadros de llamada y las líneas guía de las etiquetas de datos solo se dibujan en los gráficos de barras y de columnas.
