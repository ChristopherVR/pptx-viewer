...
Transitions and animations are approximations of PowerPoint's behaviour. Morph interpolates position, size, opacity, rotation, and colour of matching elements, and also morphs between different shape geometries: when a matched element changes shape type (or its adjustment-driven outline) between the two slides, its outline is resolved to a polygon and interpolated vertex-by-vertex into the target shape's outline, rather than crossfading. See [Limitations](/guide/limitations) for other transition and animation approximations.
