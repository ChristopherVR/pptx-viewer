...
Each published package in the monorepo is versioned and released independently: a
...
These pages are generated by `docs/sync-changelogs.mjs` from each package's historical `CHANGELOG.md` before the documentation build. Edit current
API guidance in the package documentation; do not rewrite historical release
notes to describe today's implementation. Internal packages such as `shared`,
`locales`, and `react-compat` do not have independent published releases.
