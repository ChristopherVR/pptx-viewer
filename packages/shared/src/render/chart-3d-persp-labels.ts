...
type LayoutCore = Pick<PerspChartLayout, 'view' | 'range' | 'categoryX' | 'rows' | 'kind'>;
...
export function buildPerspLabels(layout: LayoutCore, input: PerspLabelInput): PerspLabel[] {
	...
	for (const v of axisTickValues({ ...range, span })) {
		const at = perspToScreen(view, [0, ((v - range.min) / span) * h, 0]);
		labels.push({
			role: 'value',
			text: input.percent
				? `${Math.round(v * 100)}%`
	...
	input.categories.forEach((text, c) => {
		const x = layout.categoryX[c];
		if (x === undefined) {
			return;
		}
		const at = perspToScreen(view, [x, 0, 0]);
		labels.push({
			role: 'category',
			text,
			x: at.x + CATEGORY_DX,
