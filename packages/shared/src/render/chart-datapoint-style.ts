...
export interface ChartDataPointLike {
	 ...
	 * sibling `c:pictureOptions` (mirrors `PptxChartDataPoint.impliedPicture`).
	 * {@link resolveActiveDataPointPicture} falls back to this only when
	 * {@link picture} did not resolve an image of its own.
	 */
...
export interface ChartSeriesLike {
	 ...
	 * sibling `c:pictureOptions` (mirrors `PptxChartSeries.impliedPicture`).
	 */
	impliedPicture?: PptxChartDataPointPicture;
}
...
// Picture-fill resolution (c:pictureOptions, the bare-a:blipFill
// impliedPicture case, and bar3D face targeting) lives in
// chart-datapoint-picture-resolve.ts, split out to keep this file within the
// repo's ~300-LOC limit: `resolveActiveDataPointPicture`,
// `resolveDataPointPictureFill`, `resolveBarFaceTargets`, `DataPointPictureFill`,
// `BarPictureFace`, `BarFaceTargets`.
