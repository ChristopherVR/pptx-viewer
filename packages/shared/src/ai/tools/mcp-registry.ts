...
export const MCP_TOOL_ENTRIES: Record<string, McpToolEntry> = {
	...
	ole_get_content: entry(
		"Read an embedded OLE object's editable content: a spreadsheet grid, document paragraphs, or nested-deck slide titles, depending on its kind.",
		mcpSchemas.GetOleContentSchema,
		mcp.getOleContent,
		'read',
		'Read OLE content',
	),
	ole_set_sheet_cell: entry(
		"Edit one cell of an embedded Excel object's first worksheet.",
		mcpSchemas.SetOleSheetCellSchema,
		mcp.setOleSheetCell,
		'slides',
		'Edit spreadsheet cell',
	),
	ole_set_document_paragraph: entry(
		"Replace one paragraph's text in an embedded Word document object.",
		mcpSchemas.SetOleDocumentParagraphSchema,
		mcp.setOleDocumentParagraph,
		'slides',
		'Edit document paragraph',
	),
	ole_set_deck_slide_title: entry(
		'Edit one text-bearing shape on one slide of an embedded nested-presentation object (call ole_get_content first for its elementId).',
		mcpSchemas.SetOleDeckSlideTitleSchema,
		mcp.setOleDeckSlideTitle,
		'slides',
		'Edit nested slide text',
	),
	ole_replace_file: entry(
		"Replace an embedded OLE object's entire payload with a new file (works for any object kind, including a packaged/unsupported one).",
		mcpSchemas.ReplaceOleFileSchema,
		mcp.replaceOleFileT,
		'slides',
		'Replace OLE file',
	),
	ole_set_object_name: entry(
		"Rename an embedded OLE object's Object Name (p:oleObj/@name), regenerating the icon caption when shown as an icon.",
		mcpSchemas.SetOleObjectNameSchema,
		mcp.setOleObjectNameT,
		'slides',
		'Rename OLE object',
	),
