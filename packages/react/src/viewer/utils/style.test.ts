...
describe('getCssBorderDashStyle', () => {
	...
	// `border-style: double` is the only CSS border style that paints more than
	// one strand (line / gap / line, each a third of `border-width`), which is
	// exactly ECMA-376's `dbl`; the asymmetric compounds are approximated by it.
	// A compound line's strands outrank its dash pattern - CSS cannot do both.
	it('should return "double" for compound line types regardless of dash', () => {
		expect(getCssBorderDashStyle('dot', 'dbl')).toBe('double');
		expect(getCssBorderDashStyle('dash', 'thickThin')).toBe('double');
		expect(getCssBorderDashStyle('lgDash', 'thinThick')).toBe('double');
		expect(getCssBorderDashStyle('dot', 'tri')).toBe('double');
	});

	it('should ignore a single/absent compound line', () => {
		expect(getCssBorderDashStyle('dot', 'sng')).toBe('dotted');
		expect(getCssBorderDashStyle('dash', undefined)).toBe('dashed');
	});
...
describe('compound-line helpers (deprecated: `border-style: double` replaced them)', () => {
	/*
	 * The inset-box-shadow construction these two used to build could not work,
	 * and rendering it in Chromium proved it: inset shadows paint front-to-back
	 * from the padding edge inward, and the ring declared `transparent` (the
	 * "gap") paints nothing rather than punching a hole in the coloured ring
	 * under it. A `dbl` outline came out as one solid band as thick as the whole
	 * compound line, and the border was drawn at a THIRD of its authored weight
	 * to make room for strands that never appeared.
	 */
	it('never asks for a box-shadow', () => {
		expect(getCompoundLineBoxShadow('dbl', 10, '#FF0000')).toBeUndefined();
		expect(getCompoundLineBoxShadow('thickThin', 10, '#000')).toBeUndefined();
		expect(getCompoundLineBoxShadow('thinThick', 10, '#000')).toBeUndefined();
		expect(getCompoundLineBoxShadow('tri', 10, '#000')).toBeUndefined();
		expect(getCompoundLineBoxShadow('sng', 4, '#000')).toBeUndefined();
		expect(getCompoundLineBoxShadow(undefined, 4, '#000')).toBeUndefined();
	});

	it('keeps the FULL authored width on the border, which `double` then splits', () => {
		expect(getCompoundLineBorderWidth('dbl', 10)).toBe(10);
		expect(getCompoundLineBorderWidth('thickThin', 10)).toBe(10);
		expect(getCompoundLineBorderWidth('thinThick', 10)).toBe(10);
		expect(getCompoundLineBorderWidth('tri', 10)).toBe(10);
		expect(getCompoundLineBorderWidth('sng', 10)).toBe(10);
		expect(getCompoundLineBorderWidth(undefined, 10)).toBe(10);
	});

	it('styles a compound border as full-width `double`', () => {
		expect(getCompoundLineStyle('dbl', '#FF0000', 10)).toStrictEqual({
			borderWidth: 10,
			borderColor: '#FF0000',
			borderStyle: 'double',
		});
		expect(getCompoundLineStyle('tri', '#000', 6).borderStyle).toBe('double');
	});

	it('returns nothing to apply for a single / absent / unpainted line', () => {
