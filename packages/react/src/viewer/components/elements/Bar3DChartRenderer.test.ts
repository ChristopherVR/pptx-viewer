...
// jsdom/happy-dom do not provide; see ChartElementView.bar3d.test.tsx for the
// branch-selection coverage that does not require `three` to actually run).
// ---------------------------------------------------------------------------

describe('bar3DChartRenderer module', () => {
	it('exports Bar3DChartRenderer as a named export', async () => {
		...
		// itself.
		const mod = await import('./Bar3DChartRenderer');
		expectTypeOf(mod.Bar3DChartRenderer).toBeFunction();
	}, 30000);
});
