 ...
 *
 * {@link Bar3DChartRenderer} itself dynamically imports `three` and mounts a
 ...
 * `ChartElementView`: given the `BarChart3DContext` flag and the chart's
 ...
 * `ChartElementView.surface3d.test.tsx`.
 */
...
vi.mock(import('./Bar3DChartRenderer'), () => ({
	Bar3DChartRenderer: () => React.createElement('div', { 'data-testid': 'bar3d-stub' }),
}));
...
function makeBar3DElement(): ChartPptxElement {
...
describe('chartElementView - barChart3D opt-in', () => {
	it('renders the SVG path (no 3D stub) when the context flag is off', () => {
		render(makeBar3DElement(), false);
		expect(container.querySelector('[data-testid="bar3d-stub"]')).toBeNull();
		expect(container.querySelector('svg')).not.toBeNull();
	});

	it('renders the 3D stub for a bar3D chart when the context flag is on', () => {
		render(makeBar3DElement(), true);
		expect(container.querySelector('[data-testid="bar3d-stub"]')).not.toBeNull();
	});
	...
	it('defaults to the SVG path when no provider is present (context default false)', () => {
		act(() => {
			root.render(
				React.createElement(ChartElementView, { element: makeBar3DElement(), editable: false }),
			);
		});
		expect(container.querySelector('[data-testid="bar3d-stub"]')).toBeNull();
	});
