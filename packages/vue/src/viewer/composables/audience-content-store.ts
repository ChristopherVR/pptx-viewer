 ...
 * between the presenter tab and audience tab. Vue port of React's
 * `hooks/presentation-mode/audience-content-store.ts`.
 *
 * When the presenter opens an audience window, the PPTX bytes are stored in
 * IndexedDB. The audience tab retrieves them on load and then cleans up. The
 * database / store / key names match React verbatim so a Vue presenter tab and
...
/**
 * URL hash that marks a tab as the audience window. Matches React's
 * `AUDIENCE_HASH` so a Vue audience tab and a React presenter tab agree on the
 * handshake.
 */
