...
describe('smartart-node-limits', () => {
	...
	it('canAddTopLevelNode honours the layout max', () => {
		expect(canAddTopLevelNode('matrix', 4)).toBe(false);
		expect(canAddTopLevelNode('matrix', 3)).toBe(true);
		// Unbounded layout always allows adding.
		expect(canAddTopLevelNode('list', 99)).toBe(true);
	});

	it('canRemoveTopLevelNode honours the layout min', () => {
		expect(canRemoveTopLevelNode('matrix', 4)).toBe(false);
		expect(canRemoveTopLevelNode('cycle', 3)).toBe(false);
		expect(canRemoveTopLevelNode('cycle', 4)).toBe(true);
		expect(canRemoveTopLevelNode('list', 1)).toBe(false);
		expect(canRemoveTopLevelNode('list', 2)).toBe(true);
	});
