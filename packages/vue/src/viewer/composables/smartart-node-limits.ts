 ...
 *
 * Vue port of the React `smartart-node-limits.ts`. These are soft, UX-level
 * guards (not hard schema constraints): PowerPoint itself will render most
 * layouts with any number of top-level nodes, but several layouts only make
 * visual sense within a range. Surfacing the bounds lets the properties panel
 * disable add / remove and explain why, instead of silently producing a
 * broken-looking diagram.
 *
 ...
 *
 * Note: this is a shared-extraction candidate. The same table and predicates
 * exist verbatim in the React binding; both should eventually move into
 * `pptx-viewer-shared`.
 *
