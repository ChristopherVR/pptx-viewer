...
/** Injection key carrying the `pieChart3D` opt-in flag. */
export const PieChart3DKey: InjectionKey<boolean> = Symbol('pptx-pie-chart-3d');

/** Read the pie3D-chart 3D opt-in flag; defaults to `false` when not provided. */
export function usePieChart3D(): boolean {
	return inject(PieChart3DKey, false);
}
