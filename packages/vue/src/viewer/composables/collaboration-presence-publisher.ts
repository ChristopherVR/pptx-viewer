 ...
 * awareness presence.
 *
 * Publishes a single nested `presence` field over Yjs awareness, matching the
 * cross-framework wire format that React (`usePresenceTracking`) and Angular
 * consume so peers from every binding can see each other. The shape mirrors
 * `sanitizePresence` in `pptx-viewer-shared`: `{ userName, userColor,
 * userAvatar?, role?, activeSlideIndex, cursorX, cursorY, selectedElementId?,
 * lastUpdated }`.
 *
 * Outgoing updates are throttled to `BROADCAST_THROTTLE_MS` (leading edge plus
 * a trailing flush) so rapid cursor moves do not flood the network; the
...
import { BROADCAST_THROTTLE_MS } from 'pptx-viewer-shared';
