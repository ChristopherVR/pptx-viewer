/**
 * useBarChart3dScene: Vue composable that drives the shared vanilla-three
 * {@link mountBarChart3D} controller for a bar3D chart's interactive WebGL
 * view. Mirrors {@link ./useSurfaceChart3dScene.ts} exactly, swapping the
 * surface grid for a pre-built box-mesh layout ({@link BarChart3DSceneOptions}
 * from `buildBarChart3DDataForElement`).
 *
 * Responsibilities (the framework-coupled glue the SFC should stay free of):
 * - mount the shared scene into a caller-provided container ref whenever box
...
export function useBarChart3dScene(opts: UseBarChart3dSceneOptions): UseBarChart3dSceneResult {
	...
	/** Tear down the scene, then mount afresh for the current box data. */
