 ...
 * framework-agnostic math (Vue-local port of the React
 * `viewer/utils/shape-adjustment.ts`, with its `../constants`/`../types`
 * dependencies inlined; extraction into `pptx-viewer-shared` is a follow-up).
 */
import type { PptxElement, PptxElementWithShapeStyle } from 'pptx-viewer-core';
import { hasShapeProperties } from 'pptx-viewer-core';

// Constants (mirrors React `viewer/constants/scalar.ts`).
