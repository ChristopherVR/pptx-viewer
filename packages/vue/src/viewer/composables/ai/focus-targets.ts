 ...
 *
 * Framework-agnostic logic replicated from the React binding (the pure
 * `computeFocusTargets` / `focusTargetChips` / `isTwoTableFocus` helpers) so the
 * Vue panel and bridge agree on what "the current focus" is without depending on
 * a React module. Kept tiny and local to avoid a shared-package round-trip.
 */
