 ...
 * to, so no coupling to the panel composables is needed: this module talks to the
 * shared store directly. `toRenderableParts` (a pure shared helper) flattens each
 * message into text runs + tool parts. Replicated from the React binding's
 * `ai-log-export.ts`; the only framework seam is the browser download, which uses
 * the shared `downloadBlob`.
 */
