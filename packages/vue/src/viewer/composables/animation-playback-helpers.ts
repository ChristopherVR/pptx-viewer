 ...
 * a {@link PresentationAnimationController} during a Vue slide show. Ported from
 * the React binding's `presentation-mode/animation-helpers` + `build-playback`.
 *
 ...
 *    command) onto a reactive `Map<elementId, ElementAnimationState>`;
 ...
 *
 * @module composables/animation-playback-helpers
 */
...
/** Updater over the reactive element-state map (React `setState`-compatible). */
...
/** Everything the step / build / auto-advance helpers need from the composable. */
export interface PlaybackContext {
	setStates: StatesSetter;
	/** Timer ids collected here so the composable can clear them on slide change. */
	timers: number[];
...
// ---------------------------------------------------------------------------
// Media commands (p:cmd) - DOM lookup by data-element-id (no registry in Vue)
// ---------------------------------------------------------------------------
 ...
 * Mirrors the React `applyAnimationGroupSteps`.
 */
 ...
 * ordinary click-advance is unchanged. Mirrors the React `driveBuildReveal`.
 */
 ...
 * groups. Mirrors the React `scheduleAutoAdvanceChain`.
 */
