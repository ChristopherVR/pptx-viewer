...
describe('shapeEffectOverlay per-sub-path fill', () => {
	it('paints smileyFace as layered paths, with the eyes unfilled', () => {
		const wrapper = mount(ShapeEffectOverlay, { props: { element: smileyFace() } });
		const html = wrapper.html();
		expect(html).toContain('pptx-vue-subpath-fill');
	...
	it('shades the actionButtonBlank inset bevel well instead of painting it flat', () => {
		...
		const wrapper = mount(ShapeEffectOverlay, { props: { element } });
		const fills = [...wrapper.html().matchAll(/fill="([^"]+)"/gu)].map((m) => m[1]);
	...
	it('renders nothing extra for an ordinary single-fill preset (rect)', () => {
		...
		const wrapper = mount(ShapeEffectOverlay, { props: { element } });
		expect(wrapper.html()).not.toContain('pptx-vue-subpath-fill');
	});
});
