/**
 * `SmartArt3DRenderer.vue` builds its model through the shared
...
const { default: SmartArt3DRenderer } = await import('./SmartArt3DRenderer.vue');
...
describe('smartArt3DRenderer layout source', () => {
	it('resolves the 3D layout from the element data (cached drawing first)', () => {
		const i18n = createI18n({ legacy: false, locale: 'en', messages: { en: {} } });
		const wrapper = mount(SmartArt3DRenderer, {
		...
		// Vue hands the component a reactive proxy of the props, so compare by value.
		expect(source).toStrictEqual(data);
		expect(nodes).toStrictEqual(data.nodes);
