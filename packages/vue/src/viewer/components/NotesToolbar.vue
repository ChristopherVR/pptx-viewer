...
<template>
	<div class="pptx-vue-notes-toolbar mb-1 flex items-center justify-between gap-2">
		<div :class="GROUP">
			<button type="button" :class="BTN" title="Bold" @click="emit('inline', 'bold')">
				<Bold class="w-3.5 h-3.5" />
			</button>
			<button type="button" :class="BTN_L" title="Italic" @click="emit('inline', 'italic')">
				<Italic class="w-3.5 h-3.5" />
			</button>
			<button type="button" :class="BTN_L" title="Underline" @click="emit('inline', 'underline')">
			...
			<button type="button" :class="BTN" title="Bulleted list" @click="emit('toggleBullet')">
				<List class="w-3.5 h-3.5" />
			</button>
			<button type="button" :class="BTN_L" title="Numbered list" @click="emit('toggleNumbered')">
				<ListOrdered class="w-3.5 h-3.5" />
			</button>

			<span :class="SEP" aria-hidden="true" />

			<button type="button" :class="BTN" title="Increase indent" @click="emit('indent')">
				<IndentIncrease class="w-3.5 h-3.5" />
			</button>
			<button type="button" :class="BTN_L" title="Decrease indent" @click="emit('outdent')">
				<IndentDecrease class="w-3.5 h-3.5" />
			</button>

			<span :class="SEP" aria-hidden="true" />

			<button type="button" :class="BTN" title="Insert link" @click="emit('linkButtonClick')">
				<Link class="w-3.5 h-3.5" />
			</button>
			<button type="button" :class="BTN_L" title="Print notes" @click="emit('print')">
			...
			>
				<form class="space-y-2" @submit.prevent="submitLink">
					<div>
						<label class="mb-0.5 block text-[10px] text-muted-foreground">Link URL</label>
						<input
							ref="urlInputRef"
							v-model="linkUrl"
							type="text"
					...
					<div>
						<label class="mb-0.5 block text-[10px] text-muted-foreground">Display text</label>
						<input
							v-model="linkText"
							type="text"
