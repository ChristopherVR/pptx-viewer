...
function mountCursors(cursors: RemoteCursor[]) {
	return mount(CollaborationCursors, { props: { cursors } });
}
...
describe('collaborationCursors', () => {
	...
	it('positions cursors at the raw slide coordinates', () => {
		const wrapper = mountCursors([
	...
	it('ignores the deprecated zoom prop (the scaled stage applies zoom once)', async () => {
		const wrapper = mount(CollaborationCursors, {
			props: {
				cursors: [{ clientId: 'a', userName: 'Ada', color: '#123456', x: 10, y: 20 }],
				zoom: 2,
			},
		});
		const cursor = wrapper.get('[data-client-id="a"]');
