...
| **Zoom navigation**                                 | `ZoomRenderer`                                                            | Zoom element is a static link tile (no slide/section zoom navigation). Model3D interactive three.js rendering is now done (shared `mountModel3D`), poster fallback when `three` is absent.                                                           |
