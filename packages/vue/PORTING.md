...
| `useEditorHistory` (undo/redo)                                                                                                                     | ◑      | `composables/useEditorHistory.ts` — undo/redo over `Ref<PptxSlide[]>` (deep-clone snapshots, 120 cap). Not yet wired into `PowerPointViewer` (no editing UI yet)                                                                         |
| `useEditorOperations` (element CRUD/transform)                                                                                                     | ◑      | `composables/useEditorOperations.ts` — add/update/remove/transform/duplicate/reorder/updateText over the active slide via core clone helpers; snapshot-first history. Not yet wired (no editing UI). Group/align/flip/clipboard deferred |
