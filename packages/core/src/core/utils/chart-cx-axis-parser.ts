...
/** Parse `cx:axis/cx:txPr`'s default run font onto the axis formatting result. */
function parseAxisTxPr(
...
function parseSingleCxAxis(
	...
	parseAxisTxPr(xmlLookup.getChildByLocalName(axisNode, 'txPr'), xmlLookup, colorParser, result);
...
export function parseCxAxes(
	...
	const result = axisNodes.map((node) => parseSingleCxAxis(node, xmlLookup, resolvedColorParser));
