...
import { buildTransposedOrientation } from './smartart-hierarchy-transposed-orientation';
...
export function resolveHierarchyOrientation(
	...
	if (sibSpReferencesHeight || tailedTransposed) {
		return buildTransposedOrientation(constraints, itemShapeName, sibSpRatio);
	}
	...
	// SESSION 38: when nothing direct is declared, try the wrapping
	// `composite` node's own two independently-referenced `w`/`h` before
	// falling back to `DEFAULT_ASPECT_RATIO` - see `resolveIndexedAspectRatio`'s
	// own doc comment (`titled-picture-accent-list--hier5.pptx`'s own shape).
	const compositeAspect =
		findDirectAspectRatio(constraints, itemShapeName) ??
		resolveIndexedAspectRatio(algorithmNode, index) ??
		DEFAULT_ASPECT_RATIO;
