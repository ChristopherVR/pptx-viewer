 ...
 * framework code.
 *
 * Moved here from `pptx-viewer-shared` so the save/decompose pipeline (which
 * lives in `pptx-viewer-core`) can call the same interpreter the bindings
 * render with. See `smartart-layout-types.ts` for the dependency-direction
 * note.
 */
