...
export class PptxHandlerRuntime extends PptxHandlerRuntimeBase {
	public async getSmartArtDataForGraphicFrame(
		...
		// Each content node's quick-style role, coherent-3D opt-out and
		// per-presentation-node layout variables, from its `pres` points.
		const localName = (key: string) => this.compatibilityService.getXmlLocalName(key);
		const metadataOf = resolveSmartArtPointMetadata(points, parsedConnections, localName);
