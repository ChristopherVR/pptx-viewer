...
assign(
	...
	],
	{
		...
		edit: 'partial',
		serialize: 'partial',
		note: "Style and color-definition metadata are typed; complex style and color-choice payloads are preserved. Since wave 2 (W2-E), a quick-style label's own a:lnRef/a:fillRef/a:effectRef/a:fontRef are additionally resolved against the theme's fmtScheme and font scheme (core/core/runtime/smartart-style-label-refs.ts, buildSmartArtQuickStyle), reusing the same theme-ref resolvers an ordinary shape's p:style already goes through, instead of only the coarse subtle/moderate/intense enum a style label previously fell back to. Each label's own dgm:scene3d/dgm:sp3d/dgm:txPr 3D is parsed too (core/core/runtime/smartart-style-label-3d.ts) and re-applied onto the drawing shapes a structural edit regenerates (core/utils/smartart-quick-style-3d.ts, applySmartArtQuickStyle3d), matching the 3D PowerPoint bakes onto each cached shape; the whole-diagram dgm:styleDef/dgm:scene3d is PptxSmartArtQuickStyle.scene3d.",
		evidence: [
