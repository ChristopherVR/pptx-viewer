...
describe('real-world corpus round-trip', () => {
	...
	/**
	 * The corpus set is pinned by the `toStrictEqual` assertion above, so every
	 * name below is guaranteed present. These used to be `it.skipIf`, which meant
	 * a renamed deck turned five deep fidelity checks GREEN instead of red.
	 * Assert presence instead and let a missing deck fail its own test loudly.
	 */
	const requireCorpus = (name: string): string => {
		if (!fixtures.includes(name)) {
			throw new Error(
				`Missing corpus fixture ${name}. It is committed under __tests__/fixtures/corpus; ` +
					'restore it rather than skipping, and keep REQUIRED_CORPUS_FIXTURES in step.',
			);
		}
		return name;
	};

	it('smartArt + chart + table deck: chart series, table cells, and SmartArt node text survive', async () => {
		const { original, reloaded } = await loadSaveReload(
			requireCorpus('smartart-chart-table-mix.pptx'),
		);
	...
	it('master/layout deck: gradient and pattern fills survive on inherited-layout slides', async () => {
		const { original, reloaded } = await loadSaveReload(
			requireCorpus('master-layout-inheritance-fills.pptx'),
		);
	...
	it('animations/transitions deck: per-slide transition type and native animation count survive', async () => {
		const { original, reloaded } = await loadSaveReload(
			requireCorpus('animations-transitions-multislide.pptx'),
		);
	...
	it('ole/media deck: progId + embedded payload and media type survive (not "unknown"/"shape")', async () => {
		const { original, reloaded } = await loadSaveReload(requireCorpus('ole-embedded-media.pptx'));
	...
	it('preset geometry + WordArt deck: uncommon autoshape presets and text-warp geometry survive', async () => {
		const { original, reloaded } = await loadSaveReload(
			requireCorpus('preset-geometry-wordart.pptx'),
		);
