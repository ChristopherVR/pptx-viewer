...
describe('wmf-draw-handlers', () => {
	describe('handleWmfDrawRecord()', () => {
		it('is a function with arity 5', () => {
			expectTypeOf(handleWmfDrawRecord).toBeFunction();
			expect(handleWmfDrawRecord).toHaveLength(5);
		});
		...
		describe('mETA_RECTANGLE', () => {
			it('draws a filled and stroked rectangle', () => {
				...
				expect(ctx.fillRect.mock.calls[0]).toStrictEqual([5, 10, 75, 90]);
			});
		...
		describe('mETA_POLYPOLYGON', () => {
			it('draws multiple polygons', () => {
				...
				// poly1: 3 points
				let off = d + 6;
				wCtx.view.setInt16(off, 0, true);
				wCtx.view.setInt16(off + 2, 0, true);
				off += 4;
				wCtx.view.setInt16(off, 10, true);
				wCtx.view.setInt16(off + 2, 0, true);
				off += 4;
				wCtx.view.setInt16(off, 5, true);
				wCtx.view.setInt16(off + 2, 10, true);
				off += 4;
				// poly2: 2 points
				wCtx.view.setInt16(off, 20, true);
				wCtx.view.setInt16(off + 2, 20, true);
				off += 4;
				wCtx.view.setInt16(off, 30, true);
				wCtx.view.setInt16(off + 2, 30, true);
				off += 4;
