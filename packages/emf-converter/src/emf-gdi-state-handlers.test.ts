...
describe('emf-gdi-state-handlers', () => {
	describe('handleEmfGdiStateRecord()', () => {
		it('is a function with arity 5', () => {
			expectTypeOf(handleEmfGdiStateRecord).toBeFunction();
			expect(handleEmfGdiStateRecord).toHaveLength(5);
		});
		...
		describe('eMR_SAVEDC', () => {
			...
			it('unwinds clip save depth before saving', () => {
				...
				expect(
					(rCtx.ctx as unknown as Record<string, { mock: { calls: unknown[] } }>).restore,
