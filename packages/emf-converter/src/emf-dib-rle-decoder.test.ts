...
describe('decodeRleBitmap - RLE8', () => {
	...
	it('decodes RLE8 with end-of-line marker advancing to next row', () => {
		...
		const bytes = [
			2,
			0, // 2 pixels of red (index 0)
			0,
			0, // end of line
			2,
			1, // 2 pixels of green (index 1)
			0,
			1, // end of bitmap
		];
		const view = buildBuffer(bytes);
	...
	it('handles delta escape (cursor move)', () => {
		...
		const bytes = [
			...
			2, // delta escape
			2,
			0, // move right 2 pixels, 0 rows
			1,
			1, // 1 pixel of green (index 1)
			0,
			1, // end of bitmap
		];
		const view = buildBuffer(bytes);
	...
	it('returns ImageData with correct dimensions', () => {
		...
		const imageData = decodeRleBitmap(
			view,
			0,
			bytes.length,
			8,
...
describe('decodeRleBitmap - RLE4', () => {
	it('decodes an RLE4 encoded run alternating two colors', () => {
		...
		const bytes = [
			4,
			0x01, // 4 pixels: red, green, red, green
			0,
			1, // end of bitmap
		];
		const view = buildBuffer(bytes);
