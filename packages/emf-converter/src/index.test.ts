...
import { parseEmfPlusPenObject, parseEmfPlusFontObject } from './emf-plus-object-complex';
...
function writeUtf16LE(view: DataView, offset: number, str: string): void {
...
describe('emf-types', () => {
	describe('defaultState()', () => {
		...
		it('returns identity world transform', () => {
			const s = defaultState();
			expect(s.worldTransform).toStrictEqual([1, 0, 0, 1, 0, 0]);
		});
	...
	describe('cloneState()', () => {
		...
		it('deep-copies worldTransform', () => {
			...
			expect(original.worldTransform).toStrictEqual([1, 0, 0, 1, 0, 0]);
		});
	...
	describe('createEmfPlusState()', () => {
		...
		it('returns empty save stack and save ID map', () => {
			const s = createEmfPlusState();
			expect(s.saveStack).toStrictEqual([]);
...
describe('emf-canvas-helpers', () => {
	...
	describe('getStockObject()', () => {
		it('returns white brush for index 0', () => {
			...
			expect(obj).toStrictEqual({ kind: 'brush', style: 0, color: '#ffffff' });
		});

		it('returns light gray brush for index 1', () => {
			const obj = getStockObject(1);
			expect(obj).toStrictEqual({ kind: 'brush', style: 0, color: '#c0c0c0' });
		});

		it('returns gray brush for index 2', () => {
			const obj = getStockObject(2);
			expect(obj).toStrictEqual({ kind: 'brush', style: 0, color: '#808080' });
		});

		it('returns dark gray brush for index 3', () => {
			const obj = getStockObject(3);
			expect(obj).toStrictEqual({ kind: 'brush', style: 0, color: '#404040' });
		});

		it('returns black brush for index 4', () => {
			const obj = getStockObject(4);
			expect(obj).toStrictEqual({ kind: 'brush', style: 0, color: '#000000' });
		});

		it('returns null (hollow) brush for index 5', () => {
			const obj = getStockObject(5);
			expect(obj).toStrictEqual({ kind: 'brush', style: 1, color: '#000000' });
		});

		it('returns white pen for index 6', () => {
			const obj = getStockObject(6);
			expect(obj).toStrictEqual({ kind: 'pen', style: 0, widthX: 1, color: '#ffffff' });
		});

		it('returns black pen for index 7', () => {
			const obj = getStockObject(7);
			expect(obj).toStrictEqual({ kind: 'pen', style: 0, widthX: 1, color: '#000000' });
		});

		it('returns null pen for index 8', () => {
			const obj = getStockObject(8);
			expect(obj).toStrictEqual({ kind: 'pen', style: 5, widthX: 0, color: '#000000' });
		});
...
describe('emf-plus-path', () => {
	describe('parseEmfPlusPath()', () => {
		it('parses a simple compressed path (moveto + lineto)', () => {
			...
			expect(result!.points[0]).toStrictEqual({ x: 10, y: 20 });
			expect(result!.points[1]).toStrictEqual({ x: 100, y: 200 });
		...
		it('parses path with close flag on last point', () => {
			...
			const _totalSize = typesOffset + pointCount; // 24 + 3 = 27

			const view = buildBuffer(28, (v) => {
				// pad to 28 for alignment safety
				v.setUint32(0, 0xdbc01002, true);
				v.setUint32(4, pointCount, true);
		...
		it('handles non-zero offset', () => {
			...
			expect(result!.points[0]).toStrictEqual({ x: 42, y: 84 });
		});
	});
});
