 ...
 * and this adapter is all the binding contributes. Note the kebab-case keys:
 * this adapter converts the shared camelCase record for `[ngStyle]`.
 */
...
describe('angular text-body features', () => {
	it('lays a `numCol` body out in columns instead of one flex column', () => {
		...
		// `cssObjectToStyleMap` stringifies every value for `[ngStyle]`.
	...
	it('insets text into the geometry text rectangle of a chevron', () => {
		...
		expect(Number.parseFloat(String(plain['padding-left']))).toBeCloseTo(9.6, 1);
		expect(Number.parseFloat(String(chevron['padding-left']))).toBeCloseTo(59.6, 1);
		expect(Number.parseFloat(String(chevron['padding-right']))).toBeCloseTo(59.6, 1);
	});
});
