...
function svg(w: number, h: number, inner: string): string {
	return `<svg xmlns="${XMLNS}" width="${w}" height="${h}">${inner}</svg>`;
}
...
function bgRect(w: number, h: number, bg: string): string {
...
export function getPatternSvg(
	...
	const fg = fgColor;
	const bg = bgColor;
	const bg8 = bgRect(s, s, bg);

	switch (preset) {
		// ── Percentage fills ──────────────────────────────────────────────
		case 'pct5':
			return svg(s, s, bg8 + rect(0, 0, 1, 1, fg));
		case 'pct10':
			return svg(s, s, bg8 + rect(0, 0, 1, 1, fg) + rect(4, 4, 1, 1, fg));
		case 'pct20':
			return svg(4, 4, bgRect(4, 4, bg) + rect(0, 0, 1, 1, fg) + rect(2, 2, 1, 1, fg));
		case 'pct25':
			return svg(
				4,
				4,
		...
		case 'pct50':
			return svg(2, 2, bgRect(2, 2, bg) + rect(0, 0, 1, 1, fg) + rect(1, 1, 1, 1, fg));
		case 'pct60':
		...
		// ── Horizontal lines ─────────────────────────────────────────────
		case 'horz':
			return svg(s, s, bg8 + rect(0, 3, s, 2, fg));
		case 'ltHorz':
			return svg(s, s, bg8 + rect(0, 0, s, 1, fg));
		case 'dkHorz':
			return svg(s, s, bg8 + rect(0, 0, s, 4, fg));
		case 'narHorz':
			return svg(s, 4, bgRect(s, 4, bg) + rect(0, 0, s, 1, fg) + rect(0, 2, s, 1, fg));
		case 'wdHorz':
			return svg(s, 12, bgRect(s, 12, bg) + rect(0, 0, s, 2, fg));

		// ── Vertical lines ───────────────────────────────────────────────
		case 'vert':
			return svg(s, s, bg8 + rect(3, 0, 2, s, fg));
		case 'ltVert':
			return svg(s, s, bg8 + rect(0, 0, 1, s, fg));
		case 'dkVert':
			return svg(s, s, bg8 + rect(0, 0, 4, s, fg));
		case 'narVert':
			return svg(4, s, bgRect(4, s, bg) + rect(0, 0, 1, s, fg) + rect(2, 0, 1, s, fg));
		case 'wdVert':
			return svg(12, s, bgRect(12, s, bg) + rect(0, 0, 2, s, fg));

		// ── Dash lines ───────────────────────────────────────────────────
		case 'dashHorz':
			return svg(s, 4, bgRect(s, 4, bg) + rect(0, 0, 4, 1, fg));
		case 'dashVert':
			return svg(4, s, bgRect(4, s, bg) + rect(0, 0, 1, 4, fg));

		// ── Cross / grid ─────────────────────────────────────────────────
		case 'cross':
			return svg(s, s, bg8 + rect(3, 0, 2, s, fg) + rect(0, 3, s, 2, fg));
		...
		case 'dnDiag':
			return svg(
				s,
				s,
				bg8 + line(0, 0, s, s, fg, 2) + line(-s, 0, 0, s, fg, 2) + line(s, 0, s * 2, s, fg, 2),
			);
		case 'ltDnDiag':
			return svg(
				s,
				s,
				bg8 + line(0, 0, s, s, fg, 1) + line(-s, 0, 0, s, fg, 1) + line(s, 0, s * 2, s, fg, 1),
			);
		case 'dkDnDiag':
			return svg(
				s,
				s,
				bg8 + line(0, 0, s, s, fg, 3) + line(-s, 0, 0, s, fg, 3) + line(s, 0, s * 2, s, fg, 3),
			);
		case 'wdDnDiag':
			return svg(
				12,
		...
		case 'dashDnDiag':
			return svg(s, s, bg8 + line(0, 0, s, s, fg, 1, ' stroke-dasharray="4,4"'));
		...
		case 'upDiag':
			return svg(
				s,
				s,
				bg8 + line(0, s, s, 0, fg, 2) + line(-s, s, 0, 0, fg, 2) + line(s, s, s * 2, 0, fg, 2),
			);
		case 'ltUpDiag':
			return svg(
				s,
				s,
				bg8 + line(0, s, s, 0, fg, 1) + line(-s, s, 0, 0, fg, 1) + line(s, s, s * 2, 0, fg, 1),
			);
		case 'dkUpDiag':
			return svg(
				s,
				s,
				bg8 + line(0, s, s, 0, fg, 3) + line(-s, s, 0, 0, fg, 3) + line(s, s, s * 2, 0, fg, 3),
			);
		case 'wdUpDiag':
			return svg(
				12,
		...
		case 'dashUpDiag':
			return svg(s, s, bg8 + line(0, s, s, 0, fg, 1, ' stroke-dasharray="4,4"'));
		...
		// ── Checkerboard ─────────────────────────────────────────────────
		case 'smCheck':
			return svg(4, 4, bgRect(4, 4, bg) + rect(0, 0, 2, 2, fg) + rect(2, 2, 2, 2, fg));
		case 'lgCheck':
			return svg(s, s, bg8 + rect(0, 0, 4, 4, fg) + rect(4, 4, 4, 4, fg));

		// ── Grids ────────────────────────────────────────────────────────
		case 'smGrid':
			return svg(s, s, bg8 + rect(0, 0, s, 1, fg) + rect(0, 0, 1, s, fg));
		case 'lgGrid':
			return svg(16, 16, bgRect(16, 16, bg) + rect(0, 0, 16, 1, fg) + rect(0, 0, 1, 16, fg));
		case 'dotGrid':
			return svg(
				s,
				s,
		...
		case 'diagBrick':
			return svg(
				s,
				s,
				bg8 + line(0, 4, 4, 0, fg, 1) + line(4, s, s, 4, fg, 1) + line(0, s, s, 0, fg, 1),
			);

		// ── Diamond ──────────────────────────────────────────────────────
		case 'solidDmnd':
			return svg(s, s, `${bg8}<polygon points="4,0 8,4 4,8 0,4" fill="${fg}"/>`);
		case 'openDmnd':
			return svg(
				s,
				s,
		...
		case 'sphere': {
			const defs =
				...
				`</radialGradient></defs>`;
			return svg(s, s, `${bg8}${defs}<circle cx="4" cy="4" r="3.5" fill="url(#sph)"/>`);
		}
		...
		case 'shingle':
			return svg(
				s,
				s,
				bg8 + line(0, 0, s, s, fg, 1) + line(0, s, 4, 4, fg, 1) + rect(0, 7, s, 1, fg),
			);
...
export function buildPatternFillCss(
	...
	// Encode for use in a data URI. Avoid String.prototype.replaceAll for
	// compatibility with lower lib targets (ng-packagr / ES2017 baseline).
