/**
 * Unit tests for gif-export-helpers.ts (Angular viewer).
 *
 * Tests cover:
 *   - Pure planning helpers: planGifFrames, msToFrameDelayCs, clampGifDimensions
 *   - GIF89a encoder: encodeGif (structural validation, no browser APIs required)
 *
 * The encoder tests mirror packages/react/src/viewer/utils/export-gif-encoder.test.ts
 * so that both packages stay in sync.
 */
...
describe('clampGifDimensions', () => {
	it('leaves dimensions unchanged when both are within the limit', () => {
		const result = clampGifDimensions(800, 600);
		expect(result).toStrictEqual({ width: 800, height: 600 });
	});

	it('leaves dimensions unchanged at exactly the limit', () => {
		const result = clampGifDimensions(1920, 1080);
	...
	it('returns whole-pixel (floor) values', () => {
		// 3000 × 2000 with maxSide=1024 → ratio = 1024/3000 ≈ 0.341
		// width = floor(1024) = 1024, height = floor(2000 * 0.341) = 682
...
describe('encodeGif', () => {
	...
	it('encodes multiple frames and produces larger output than a single frame', () => {
		...
		const multi = encodeGif([f1, f2, f3]);
		const single = encodeGif([f1]);
		expect(multi.length).toBeGreaterThan(single.length);
		const header = String.fromCharCode(...multi.slice(0, 6));
		expect(header).toBe('GIF89a');
	...
	it('encodes the delay centiseconds in the Graphic Control Extension', () => {
		const frame = { imageData: makeImageData(2, 2, 0, 0, 0), width: 2, height: 2 };
		const result = encodeGif([frame], { delayCs: 100 });

		// Locate GCE: 0x21 0xF9 0x04
		let gceIdx = -1;
	...
	it('sets the local colour table flag in each image descriptor', () => {
		...
		// Packed byte is at imgDescIdx + 9; bit 7 = local colour table present
	...
	it('uses loopCount = 0 (loop forever) by default', () => {
		const frame = { imageData: makeImageData(2, 2, 0, 0, 0), width: 2, height: 2 };
		const result = encodeGif([frame]);
		// Loop count is encoded after "NETSCAPE2.0\x03\x01" (two little-endian bytes)
		const asString = Array.from(result)
			.map((b) => String.fromCharCode(b))
			.join('');
		const nsIdx = asString.indexOf('NETSCAPE2.0');
		expect(nsIdx).toBeGreaterThan(0);
		// Loop count at nsIdx + 11 (after 11-char identifier) + 2 (sub-block 03 01)
