/**
 * Drawing-shape view-model helpers for the Angular SmartArt renderer.
 *
 ...
 * independent of the shared SVG-fallback layout engine
 * (`computeSmartArtLayout`), which only runs when no drawing shapes exist.
 *
 * Pure TypeScript (no Angular imports) so it unit-tests with vitest without
 * TestBed. Style helpers (`truncate`, `styleStroke`, `styleShadow`) are reused
 * from `pptx-viewer-shared` to stay aligned with the other bindings.
 */
import type {
	PptxSmartArtChrome,
	PptxSmartArtData,
...
import { styleShadow, styleStroke, truncate } from '../internal/shared';
