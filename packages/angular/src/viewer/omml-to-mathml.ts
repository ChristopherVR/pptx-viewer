 ...
 */
export function ommlToMathml(omml: Record<string, unknown> | string | undefined): string {
	if (omml === undefined || typeof omml === 'string') {
	...
	// `equationXml` is typed as `Record<string, unknown>` on the core segment;
	// the converter walks it as an `OmmlNode` (core's recursive `XmlObject`).
	return convertOmmlToMathMl(omml as OmmlNode);
}
