 ...
 *
 * Replicated from the React binding's `hooks/ai/focus-targets.ts` (kept as a
 * tiny per-binding copy rather than editing `pptx-viewer-shared`, per the port
 * ground rules). Shared by the panel's focus chips + the merge-tables directive
 * so both agree on what "the current focus" is.
 */
