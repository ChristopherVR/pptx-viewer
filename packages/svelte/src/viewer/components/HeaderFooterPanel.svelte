...
<div class="backdrop"><button class="scrim" type="button" aria-label={t('pptx.headerFooter.close')} onclick={onclose}></button>
	...
	<section role="dialog" tabindex="-1" aria-modal="true" aria-labelledby="hf-title"><header><h2 id="hf-title">{t('pptx.headerFooter.title')}</h2><button type="button" aria-label={t('pptx.headerFooter.close')} onclick={onclose}>×</button></header>
		<div class="body"><label><input type="checkbox" checked={draft.hasDateTime ?? false} onchange={(event) => toggle('hasDateTime', event.currentTarget.checked)} />◷ {t('pptx.headerFooter.dateAndTime')}</label><label><input type="checkbox" checked={draft.hasSlideNumber ?? false} onchange={(event) => toggle('hasSlideNumber', event.currentTarget.checked)} /># {t('pptx.headerFooter.slideNumber')}</label><label><input type="checkbox" checked={draft.hasFooter ?? false} onchange={(event) => toggle('hasFooter', event.currentTarget.checked)} />▤ {t('pptx.headerFooter.footer')}</label>{#if draft.hasFooter}<input type="text" aria-label={t('pptx.headerFooter.footer')} placeholder={t('pptx.headerFooter.footerPlaceholder')} value={draft.footerText ?? ''} oninput={(event) => (draft = { ...draft, footerText: event.currentTarget.value })} />{/if}</div>
