<script lang="ts">
	 ...
	 * `mountAreaChart3D` `interaction` callbacks (`Chart3DInteractionController`,
	...
	const interactionController = new Chart3DInteractionController<AreaChart3DHandle>({
	...
	async function mountScene(
		...
		const result = editable
			...
			: await mountAreaChart3D(sceneHost, { ...options, textStyle });
...
<style>
	...
	.pptx-svelte-area-chart-3d-drag-badge {
		position: absolute;
		top: 4px;
		right: 4px;
		padding: 1px 6px;
