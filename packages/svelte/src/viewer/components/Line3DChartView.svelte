<script lang="ts">
	/**
	 * Line3DChartView: Svelte port of the interactive Three.js line3D-chart
	 * renderer. Mirrors `Bar3DChartView.svelte` exactly, swapping the box-mesh
	 * layout for the per-series tube-path layout (`buildLineChart3DDataForElement`
	 * / `mountLineChart3D` from `pptx-viewer-shared`).
	 *
	...
	import { buildLineChart3DDataForElement, canDrillDown, mountLineChart3D, shouldRenderHitTarget } from 'pptx-viewer-shared';
	...
	/**
	 * See `Bar3DChartView`'s `chartEditable` doc; identical gate for a line3D
	...
	async function mountScene(
		version: number,
		options: ReturnType<typeof buildLineChart3DDataForElement>,
	): Promise<void> {
		if (!options) {
			return;
		}
...
{#if mounted}
	...
	<div
		class={`pptx-svelte-element pptx-svelte-line-chart-3d${chartEditable ? ' pptx-chart-interactive' : ''}`}
...
<style>
	.pptx-svelte-line-chart-3d-scene {
		width: 100%;
		height: 100%;
