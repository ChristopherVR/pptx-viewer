<script lang="ts">
	...
	import { buildSurfaceChart3DDataForElement, canDrillDown, mountSurfaceChart3D, shouldRenderHitTarget } from 'pptx-viewer-shared';
	...
	/**
	 * See `Bar3DChartView`'s `chartEditable` doc; identical gate for a
	 * surface3D chart. Named apart from the `editable` prop above, which gates
...
{#if mounted}
	...
	<div
		class={`pptx-svelte-element pptx-svelte-surface-chart-3d${chartEditable ? ' pptx-chart-interactive' : ''}`}
