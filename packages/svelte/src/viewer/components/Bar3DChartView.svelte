<script lang="ts">
	...
	import { buildBarChart3DDataForElement, canDrillDown, mountBarChart3D, shouldRenderHitTarget } from 'pptx-viewer-shared';
	 ...
	 * matching 2D `ChartView`'s `chartEditable` gate: only on the editable
	 * canvas (`onchartpointcommit` present) and never when
	 * `p:graphicFrameLocks/@noDrilldown` forbids entering this chart's
	 * individual parts. Named apart from the `editable` prop above, which
	 * gates the degenerate-element hit target instead (issue #285).
	 */
...
{#if mounted}
	...
	<div
		class={`pptx-svelte-element pptx-svelte-bar-chart-3d${chartEditable ? ' pptx-chart-interactive' : ''}`}
