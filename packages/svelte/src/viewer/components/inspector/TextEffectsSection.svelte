...
<details>
	...
	<label class="check"><pptx-ui-checkbox checked={Boolean(style.textShadowColor)} onchange={(event) => patch(event.currentTarget.checked ? { textShadowColor: '#000000', textShadowBlur: 4, textShadowOffsetX: 2, textShadowOffsetY: 2, textShadowOpacity: 0.5 } : { textShadowColor: undefined, textShadowBlur: undefined, textShadowOffsetX: undefined, textShadowOffsetY: undefined, textShadowOpacity: undefined })} ></pptx-ui-checkbox>Shadow</label>
	...
	<label class="check"><pptx-ui-checkbox checked={Boolean(style.textGlowColor)} onchange={(event) => patch(event.currentTarget.checked ? { textGlowColor: '#ffff00', textGlowRadius: 6, textGlowOpacity: 0.6 } : { textGlowColor: undefined, textGlowRadius: undefined, textGlowOpacity: undefined })} ></pptx-ui-checkbox>Glow</label>
	...
	<label>Warp<pptx-ui-select aria-label="Warp" value={style.textWarpPreset ?? 'none'} onchange={(event) => patch({ textWarpPreset: event.currentTarget.value === 'none' ? undefined : event.currentTarget.value as TextStyle['textWarpPreset'] })}><option value="none">None</option>{#each warpPresets as preset}<option value={preset}>{warpPresetLabel(preset)}</option>{/each}</pptx-ui-select></label>
	...
	<label class="check"><pptx-ui-checkbox checked={Boolean(style.text3d)} onchange={(event) => patch({ text3d: event.currentTarget.checked ? { extrusionHeight: 57150, extrusionColor: '#808080', presetMaterial: 'plastic', bevelTopType: 'circle', bevelTopWidth: 25400, bevelTopHeight: 25400 } : undefined })} ></pptx-ui-checkbox>3D and bevel</label>
	{#if style.text3d}<div class="grid"><label>Depth (EMU)<input type="number" min="0" value={style.text3d.extrusionHeight ?? 0} onchange={(event) => patch3d({ extrusionHeight: Number(event.currentTarget.value) })} /></label><label>Extrusion<input type="color" value={style.text3d.extrusionColor ?? '#808080'} onchange={(event) => { patch3d({ extrusionColor: event.currentTarget.value }); pushColor(event.currentTarget.value); }} /></label><label>Material<pptx-ui-select aria-label="Material" value={style.text3d.presetMaterial ?? 'plastic'} onchange={(event) => patch3d({ presetMaterial: event.currentTarget.value as Text3DStyle['presetMaterial'] })}>{#each materials as material}<option value={material}>{materialPresetLabel(material)}</option>{/each}</pptx-ui-select></label><label>Bevel<pptx-ui-select aria-label="Bevel" value={style.text3d.bevelTopType ?? 'circle'} onchange={(event) => patch3d({ bevelTopType: event.currentTarget.value as Text3DStyle['bevelTopType'] })}>{#each bevels as bevel}<option value={bevel}>{bevelPresetLabel(bevel)}</option>{/each}</pptx-ui-select></label></div>{/if}
</details>
