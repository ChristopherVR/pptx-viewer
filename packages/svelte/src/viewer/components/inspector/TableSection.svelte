...
{#if table}
	...
	<div class="structure"><label>Row<input type="number" min="1" max={table.rows.length} value={activeRow + 1} onchange={(event) => (activeRow = Math.max(0, Number(event.currentTarget.value) - 1))} /></label><button type="button" onclick={() => patchData(insertTableRow(table, activeRow + 1))}>Insert row</button><button type="button" disabled={table.rows.length <= 1} onclick={() => patchData(deleteTableRow(table, activeRow))}>Delete row</button><label>Column<input type="number" min="1" max={table.columnWidths.length} value={activeColumn + 1} onchange={(event) => (activeColumn = Math.max(0, Number(event.currentTarget.value) - 1))} /></label><button type="button" onclick={() => patchData(insertTableColumn(table, activeColumn + 1))}>Insert column</button><button type="button" disabled={table.columnWidths.length <= 1} onclick={() => patchData(deleteTableColumn(table, activeColumn))}>Delete column</button></div>
