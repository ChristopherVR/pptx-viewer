...
{#if data}<div class="section">
	...
	<div class="checks"><label><pptx-ui-checkbox checked={data.style?.hasLegend ?? false} onchange={(event) => patch({ style: { ...data.style, hasLegend: event.currentTarget.checked } })} ></pptx-ui-checkbox>Legend</label><label><pptx-ui-checkbox checked={data.style?.hasDataLabels ?? false} onchange={(event) => patch({ style: { ...data.style, ...chartDataLabelsTogglePatch(data.style, event.currentTarget.checked) } })} ></pptx-ui-checkbox>Data labels</label><label><pptx-ui-checkbox checked={data.style?.hasGridlines ?? false} onchange={(event) => patch({ style: { ...data.style, hasGridlines: event.currentTarget.checked } })} ></pptx-ui-checkbox>Gridlines</label>
		...
		{#if data.chartType === 'radar'}
			<label>{t('pptx.chart.radarStyleLabel')}<pptx-ui-select aria-label={t('pptx.chart.radarStyleLabel')} data-testid="pptx-chart-radar-style" value={data.radarStyle ?? 'standard'} onchange={(event) => onRadarStyleChange(event.currentTarget.value)}>{#each RADAR_STYLE_OPTIONS as option (option.value)}<option value={option.value}>{t(option.labelKey)}</option>{/each}</pptx-ui-select></label>
		{/if}
		{#if data.chartType === 'surface'}
			<label>{t('pptx.chart.surfaceWireframeLabel')}<pptx-ui-select aria-label={t('pptx.chart.surfaceWireframeLabel')} data-testid="pptx-chart-surface-wireframe" value={data.wireframe ? 'true' : 'false'} onchange={(event) => onSurfaceWireframeChange(event.currentTarget.value)}>{#each SURFACE_WIREFRAME_OPTIONS as option (option.value)}<option value={option.value}>{t(option.labelKey)}</option>{/each}</pptx-ui-select></label>
		{/if}
	</div>
	<ChartDataGrid
		{data}
