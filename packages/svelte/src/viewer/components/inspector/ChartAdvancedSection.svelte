<script lang="ts">
	...
	function markerFillPatch(index: number, color: string): void {
		const series = data.series[index];
		seriesPatch(index, { marker: { ...series.marker!, spPr: { ...series.marker!.spPr, fillColor: color } } });
		editor.recordRecentColor(color);
	}

	function pointFillPatch(index: number, pointIndex: number, color: string): void {
		const point = data.series[index].dataPoints?.find((p) => p.idx === pointIndex);
		pointPatch(index, pointIndex, { spPr: { ...point?.spPr, fillColor: color } });
		editor.recordRecentColor(color);
	}
...
<details><summary>Series options</summary>
	{#each data.series as series, index}<fieldset><legend>{series.name}</legend>
		<label>Series chart type<pptx-ui-select aria-label="Series chart type" value={series.seriesChartType ?? ''} onchange={(event) => seriesPatch(index, { seriesChartType: (event.currentTarget.value || undefined) as PptxChartType | undefined })}><option value="">Chart default</option>{#each seriesChartTypes as type}<option value={type}>{schemaLabel(CHART_TYPE_LABEL_KEYS, type, t)}</option>{/each}</pptx-ui-select></label>
		<div class="grid"><label>Marker<pptx-ui-select aria-label="Marker" value={series.marker?.symbol ?? ''} onchange={(event) => seriesPatch(index, { marker: event.currentTarget.value ? { ...series.marker, symbol: event.currentTarget.value as NonNullable<PptxChartSeries['marker']>['symbol'] } : undefined })}><option value="">None</option>{#each markerSymbols as marker}<option value={marker}>{schemaLabel(CHART_MARKER_SYMBOL_LABEL_KEYS, marker, t)}</option>{/each}</pptx-ui-select></label>{#if series.marker}<label>Marker size<input type="number" min="2" max="72" value={series.marker.size ?? 6} onchange={(event) => seriesPatch(index, { marker: { ...series.marker!, size: Number(event.currentTarget.value) } })} /></label><label>Marker fill<input type="color" value={series.marker.spPr?.fillColor ?? series.color ?? '#4472c4'} onchange={(event) => markerFillPatch(index, event.currentTarget.value)} /></label>{/if}</div>
	</fieldset>{/each}
</details>

{#if data.categories.length && data.series.length}<details><summary>Data points</summary>
	{#if data.series.length > 1}<label>Series<pptx-ui-select aria-label="Series" value={pointSeries} onchange={(event) => pointSeries = Number(event.currentTarget.value)}>{#each data.series as series, index}<option value={index}>{series.name}</option>{/each}</pptx-ui-select></label>{/if}
	{#each data.categories as category, index}{#if activeSeries}<div class="point"><span title={category}>{category}</span><input aria-label="Point label" placeholder="Label" value={activeSeries.dataLabels?.find((label)=>label.idx===index)?.text ?? ''} onchange={(event) => labelPatch(pointSeries,index,event.currentTarget.value)} /><input type="color" aria-label="Point fill" value={activeSeries.dataPoints?.find((point)=>point.idx===index)?.spPr?.fillColor ?? activeSeries.color ?? '#4472c4'} onchange={(event) => pointFillPatch(pointSeries, index, event.currentTarget.value)} />{#if ['pie','doughnut'].includes(data.chartType)}<input type="number" min="0" max="100" aria-label="Explosion" value={activeSeries.dataPoints?.find((point)=>point.idx===index)?.explosion ?? 0} onchange={(event) => pointPatch(pointSeries,index,{explosion:Number(event.currentTarget.value)})} />{/if}</div>{/if}{/each}
</details>{/if}
