...
{#if media}<div class="section">
	...
	<div class="checks"><label><pptx-ui-checkbox checked={media.autoPlay ?? false} onchange={(event) => patch({ autoPlay: event.currentTarget.checked })} ></pptx-ui-checkbox>Auto play</label><label><pptx-ui-checkbox checked={media.loop ?? false} onchange={(event) => patch({ loop: event.currentTarget.checked })} ></pptx-ui-checkbox>Loop</label><label><pptx-ui-checkbox checked={media.fullScreen ?? false} onchange={(event) => patch({ fullScreen: event.currentTarget.checked })} ></pptx-ui-checkbox>Full screen</label><label><pptx-ui-checkbox checked={media.playAcrossSlides ?? false} onchange={(event) => patch({ playAcrossSlides: event.currentTarget.checked, ...(event.currentTarget.checked ? { autoPlay: true } : {}) })} ></pptx-ui-checkbox>Across slides</label><label><pptx-ui-checkbox checked={media.hideWhenNotPlaying ?? false} onchange={(event) => patch({ hideWhenNotPlaying: event.currentTarget.checked })} ></pptx-ui-checkbox>Hide when stopped</label></div>
	...
	<label>Speed <pptx-ui-select aria-label="Speed" value={media.playbackSpeed ?? 1} onchange={(event) => patch({ playbackSpeed: Number(event.currentTarget.value) })}>{#each [0.25,0.5,0.75,1,1.25,1.5,2,3,4] as speed}<option value={speed}>{speed}x</option>{/each}</pptx-ui-select></label>
