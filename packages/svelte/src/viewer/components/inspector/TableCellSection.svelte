...
<details open><summary>Cell formatting</summary>
	...
	{#if cell}<div class="grid"><label>Font size<input type="number" min="6" max="200" value={style.fontSize ?? 14} onchange={(event) => patchStyle({ fontSize: Number(event.currentTarget.value) })} /></label><label>Text color<input type="color" value={style.color ?? '#000000'} onchange={(event) => patchStyle({ color: event.currentTarget.value })} /></label><label>Background<input type="color" value={style.backgroundColor ?? '#ffffff'} onchange={(event) => patchStyle({ backgroundColor: event.currentTarget.value, fillMode: 'solid' })} /></label><label>Fill<select aria-label="Fill" value={style.fillMode ?? 'solid'} onchange={(event) => setFillMode(event.currentTarget.value as PptxTableCellStyle['fillMode'])}><option value="solid">Solid</option><option value="gradient">Gradient</option><option value="pattern">Pattern</option><option value="none">None</option></select></label></div>
