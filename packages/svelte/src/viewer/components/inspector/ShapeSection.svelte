...
{#if el.type === 'connector'}
	<div class="grid"><label>Start arrow<select value={style?.connectorStartArrow ?? 'none'} onchange={(event) => patchStyle({ connectorStartArrow: event.currentTarget.value as ConnectorArrowType })}>{#each arrows as arrow}<option value={arrow}>{arrow}</option>{/each}</select></label><label>End arrow<select value={style?.connectorEndArrow ?? 'none'} onchange={(event) => patchStyle({ connectorEndArrow: event.currentTarget.value as ConnectorArrowType })}>{#each arrows as arrow}<option value={arrow}>{arrow}</option>{/each}</select></label></div>
{/if}
