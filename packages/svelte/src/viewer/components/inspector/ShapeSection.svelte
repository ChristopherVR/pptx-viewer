...
{#if el.type === 'shape'}
	<label>Shape type<pptx-ui-select aria-label="Shape type" value={shapeType ?? 'rect'} onchange={(event) => setShapeType(event.currentTarget.value)}>{#each SHAPE_PRESET_DEFS as preset}<option value={preset.type}>{preset.label}</option>{/each}</pptx-ui-select></label>
{/if}
