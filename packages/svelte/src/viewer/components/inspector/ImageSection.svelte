...
<label class="pptx-svelte-field-checkbox"><pptx-ui-checkbox checked={Boolean(effects?.colorWash)} onchange={(event) => setEffects({ colorWash: event.currentTarget.checked ? { color: '#0066cc', opacity: 40 } : undefined })} ></pptx-ui-checkbox><span>Color wash</span></label>
