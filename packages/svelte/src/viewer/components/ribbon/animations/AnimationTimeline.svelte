<script lang="ts">
	...
	import { DIRECTION_VALUES, REPEAT_MODE_VALUES, schemaLabel, SEQUENCE_VALUES, TIMING_CURVE_VALUES, TRIGGER_VALUES } from 'pptx-viewer-shared';
...
{#if animations.length}<div class="timeline"><h4>Animation Timeline</h4>{#each animations as animation, index (animation.elementId)}<div role="listitem" class:selected={animation.elementId === editor.selectedElementId} draggable="true" ondragstart={() => (draggingId = animation.elementId)} ondragover={(event) => event.preventDefault()} ondrop={() => drop(animation.elementId)}>
	...
	<select aria-label="Repeat" value={animation.repeatMode ?? 'none'} onchange={(event) => replace(animation.elementId, { repeatMode: event.currentTarget.value === 'none' ? undefined : event.currentTarget.value as PptxAnimationRepeatMode })}>{#each REPEAT_MODE_VALUES as repeat}<option value={repeat}>{schemaLabel(REPEAT_MODE_LABEL_KEYS, repeat, t)}</option>{/each}</select>
...
<style>.timeline{display:grid;flex-basis:100%;gap:4px;padding-top:6px;border-top:1px solid var(--pptx-border)}h4{margin:0;font-size:10px;text-transform:uppercase}.timeline>div{display:grid;grid-template-columns:minmax(130px,1fr) repeat(5,minmax(84px,auto)) repeat(3,64px);gap:4px;align-items:end;padding:4px;border-radius:5px;background:var(--pptx-muted);font-size:9px}.timeline>div.selected{box-shadow:inset 3px 0 var(--pptx-primary)}.target{display:inline-flex;align-items:center;gap:4px;text-align:left}label{display:grid;gap:2px;color:var(--pptx-muted-foreground)}input,select,button{min-width:0;height:24px;border:1px solid var(--pptx-border);border-radius:4px;background:var(--pptx-background);color:inherit;font-size:9px}</style>
