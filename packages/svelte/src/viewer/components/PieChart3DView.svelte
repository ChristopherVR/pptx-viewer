<script lang="ts">
	...
	import { buildPieChart3DDataForElement, canDrillDown, mountPieChart3D, shouldRenderHitTarget } from 'pptx-viewer-shared';
	...
	/**
	 * See `Bar3DChartView`'s `chartEditable` doc; identical gate for a pie3D
	 * chart's wedges. Named apart from the `editable` prop above, which gates
...
{#if mounted}
	...
	<div
		class={`pptx-svelte-element pptx-svelte-pie-chart-3d${chartEditable ? ' pptx-chart-interactive' : ''}`}
