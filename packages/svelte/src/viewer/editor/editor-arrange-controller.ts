...
export class EditorArrangeController {
	...
	/** Commit `compute(slides)` (with history) only when it returns a new array. */
	#mutate(compute: (slides: EditorState['slides']) => EditorState['slides'] | null): void {
		const next = compute(this.#editor.slides);
		if (next) {
	...
	alignSelected(edge: AlignEdge): void {
		...
		this.#mutate((slides) =>
			alignSelectedOnSlide(slides, this.#editor.currentSlideIndex, ids, edge),
		);
	}
	...
	distributeSelected(axis: DistributeAxis): void {
		const ids = this.#editor.selection.ids;
		this.#mutate((slides) =>
			distributeSelectedOnSlide(slides, this.#editor.currentSlideIndex, ids, axis),
		);
	}
	...
	flipSelected(axis: 'horizontal' | 'vertical'): void {
		const ids = this.#editor.selection.ids;
		this.#mutate((slides) =>
			flipSelectedOnSlide(slides, this.#editor.currentSlideIndex, ids, axis),
		);
	}
	...
	groupSelected(): void {
		...
		this.#mutate((slides) => {
			const result = groupSelectedOnSlide(slides, this.#editor.currentSlideIndex, ids);
	...
	ungroupSelected(): void {
		...
		let childIds: string[] = [];
		this.#mutate((slides) => {
			const result = ungroupOnSlide(slides, this.#editor.currentSlideIndex, id);
