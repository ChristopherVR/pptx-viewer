...
Install `three` only when you enable a 3D SmartArt or 3D chart option. The viewer falls back to
...
| `ai`                                                                       | `PptxAiConfig`                                        | -                      | Optional AI assistant configuration. The SDK peer loads only when its panel is opened.                                                    |
| `autosave`                                                                 | `boolean`                                             | `true`                 | Debounced crash-recovery autosave to IndexedDB.                                                                                           |
| `autosaveIntervalMs`                                                       | `number`                                              | File > Options cadence | Autosave debounce window.                                                                                                                 |
...
```ts
import { translationsZhCN } from 'pptx-vanilla-viewer/i18n/zh-CN';
```
