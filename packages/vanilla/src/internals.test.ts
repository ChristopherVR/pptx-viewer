/**
 * Export-surface test for `pptx-vanilla-viewer/internals` (issue #290).
 *
 * A consumer embedding their own presentation surface needs the framework-
 * neutral slide-transition resolver/keyframes and the DOM-level overlay
 * driver that calls them, without reaching into `pptx-viewer-shared` (a
 * private, unpublished workspace package). This asserts the whole surface is
 * actually importable from `./internals`.
 */
import { describe, expect, it } from 'vitest';

import * as internals from './internals';

describe('pptx-vanilla-viewer/internals: slide-transition helpers', () => {
	...
	it('exports the DOM-level transition overlay driver', () => {
		expect(internals.playTransitionOverlay).toBeTypeOf('function');
	});
});
