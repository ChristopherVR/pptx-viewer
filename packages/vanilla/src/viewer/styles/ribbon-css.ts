...
.pptxv-bs-back-row{display:flex;height:40px;flex:none;border-bottom:1px solid var(--pptx-border,#ddd)}
.pptxv-bs-back{width:100%;height:40px;border-bottom:0}
.pptxv-backstage nav{padding:0 0 8px}
/* Mobile: stack the backstage and turn the vertical rail into a horizontal,
   scrollable tab bar with a bottom indicator, and compact the content pane.
   Same breakpoint as the rest of the mobile chrome (mobile-sheet-css.ts). */
@media (max-width: 767px), (max-width: 1023px) and (max-height: 520px) {
	.pptxv-backstage { flex-direction: column; }
	.pptxv-backstage > aside { width: 100%; flex-direction: row; align-items: center; overflow-x: auto; border-right: 0; border-bottom: 1px solid var(--pptx-border, #d7d7d7); }
	.pptxv-bs-back { min-width: 48px; flex: none; height: 48px; border-bottom: 0; border-right: 1px solid var(--pptx-border, #ddd); }
	.pptxv-bs-back-row { width: 48px; height: 48px; border-bottom: 0; border-right: 1px solid var(--pptx-border, #ddd); }
	.pptxv-bs-back { width: 48px; height: 48px; border-right: 0; }
	.pptxv-backstage nav { flex-direction: row; align-items: center; flex-wrap: nowrap; overflow-y: visible; padding: 0; }
	.pptxv-backstage nav i { display: none; }
	.pptxv-backstage nav button { flex: none; white-space: nowrap; border-left: 0; border-bottom: 2px solid transparent; padding: 0 12px; }
	.pptxv-backstage nav button.active { border-left-color: transparent; border-bottom-color: var(--pptx-primary, #c43e1c); }
	.pptxv-backstage main { padding: 16px; }
}
`;
