 ...
 * short chip labels shown in the AI focus bar. Replicated (framework-free) from
 * React's `hooks/ai/focus-targets.ts` so the Vanilla focus bar and bridge agree
 * on what "the current focus" is, identically to the other bindings.
 */
 ...
 * useful disambiguator, so `Shape 9` reads far better than the raw id.
 */
...
export function isTwoTableFocus(
	...
	if (targets.length !== 2 || targets.some((tg) => tg.kind !== 'element')) {
		return false;
	}
...
/** Directive that fires `merge_tables` without a confirmation round-trip. */
