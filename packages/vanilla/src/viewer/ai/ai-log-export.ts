/**
 * Detailed AI chat-log export for technical / debugging use. The AI settings
 * section lets technical users download the full transcript of every stored
 * assistant chat, INCLUDING each tool call's name, input, state and
 * output/error text (the detail the panel only surfaces in collapsed cards).
 * The primary artifact is machine-readable JSON; a Markdown transcript is also
 * offered. Replicated (framework-free) from React's `utils/ai-log-export.ts`;
 * reads from the same shared {@link createChatHistoryStore} the panel persists
 * to and flattens messages with the shared {@link toRenderableParts}.
 */
...
export function buildChatLogExport(
	...
	return {
		format: 'pptx-ai-chat-log',
		version: 1,
		exportedAt: isoOf(options?.now ?? Date.now()),
		detailed,
...
export function buildChatLogMarkdown(doc: AiLogExport): string {
	...
	for (const chat of doc.chats) {
		...
		out.push(`- created: ${chat.createdAtIso}`, `- updated: ${chat.updatedAtIso}`);
