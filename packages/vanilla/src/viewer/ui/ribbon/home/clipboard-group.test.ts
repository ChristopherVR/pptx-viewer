...
function handlers() {
	return { copy: vi.fn(), cut: vi.fn(), paste: vi.fn(), duplicate: vi.fn(), delete: vi.fn() };
}
...
describe('createClipboardGroup', () => {
	it('dispatches each handler from its button', () => {
		...
		const [paste, cut, copy, duplicate, del] =
	...
	it('gates paste on hasClipboard, others on hasSelection/editable', () => {
		...
		group.update({ hasSelection: false, hasClipboard: false, editable: true });
		...
		group.update({ hasSelection: true, hasClipboard: true, editable: true });
		...
		group.update({ hasSelection: true, hasClipboard: true, editable: false });
		expect(copy.disabled).toBeFalsy();
		expect(cut.disabled).toBeTruthy();
		expect(paste.disabled).toBeTruthy();
	});
