 ...
 * transforms, unsupported features) to the cloned capture document. Mirrors
 * the React/Angular `renderToCanvas` wrapper (`packages/react/src/lib/canvas-export.ts`,
 * `packages/angular/src/lib/canvas-export.ts`); the pure colour/CSS passes
 * live once in `pptx-viewer-shared`, only this thin html2canvas-pro glue is
 * per-binding. `html2canvas-pro` is imported dynamically so it stays out of
 * the main bundle until export is actually used.
 */
