...
describe('createExportController', () => {
	...
	it('exports every slide into a multi-page PDF', async () => {
		const rasterizeSlide = vi.fn().mockResolvedValue(fakeCanvas()),
			{ exportPdf } = createExportController({ store: makeStore(3), rasterizeSlide });
		await exportPdf();
	...
	it('does nothing when there are no slides', async () => {
		const rasterizeSlide = vi.fn().mockResolvedValue(fakeCanvas()),
			{ exportPdf } = createExportController({ store: makeStore(0), rasterizeSlide });
		await exportPdf();
		expect(rasterizeSlide).not.toHaveBeenCalled();
	...
	it('cancels a PDF export mid-loop once the signal aborts', async () => {
		const controller = new AbortController(),
			...
			{ exportPdf } = createExportController({ store: makeStore(4), rasterizeSlide });
	...
	it('ignores a second export while one is already running', async () => {
		...
		const rasterizeSlide = vi.fn().mockImplementation(
			...
			{ exportSlidePng } = createExportController({ store: makeStore(2), rasterizeSlide }),
			first = exportSlidePng(0),
			second = exportSlidePng(1);
		resolveFirst?.();
	...
	it('prints via the injected print-window opener', async () => {
		const rasterizeSlide = vi.fn().mockResolvedValue(fakeCanvas()),
			openPrintWindow = vi.fn((_html: string) => true),
			{ print } = createExportController({ store: makeStore(2), rasterizeSlide });
	...
	it('routes GIF/video/print through the single-export guard', async () => {
		...
		const rasterizeSlide = vi.fn().mockImplementation(
			...
			openPrintWindow = vi.fn((_html: string) => true),
			controller = createExportController({ store: makeStore(1), rasterizeSlide }),
			first = controller.exportSlidePng(0),
			gif = controller.exportGif(),
			video = controller.exportVideo(),
			print = controller.print({ openPrintWindow });
		resolveFirst?.();
	...
	it('exports the deck as a pptx-viewer-json download named after the source file', () => {
		...
		const orig = document.createElement.bind(document),
			...
			{ exportJson } = createExportController({
				store: makeStore(2),
				rasterizeSlide,
