/**
 * `buildSmartArt3DElementModel` tests: the 3D model is built from the same
 * geometry the SVG renderer draws, so the cached `dsp:` drawing (with the
 * deck's theme fills) wins over the layout engine when present.
 */
import type { PptxElement, PptxSmartArtData } from 'pptx-viewer-core';
import { describe, expect, it } from 'vitest';

import { buildSmartArt3DElementModel } from './smartart-3d';
...
describe('buildSmartArt3DElementModel', () => {
	it('returns null for a non-smartArt element or an empty diagram', () => {
		expect(buildSmartArt3DElementModel({ id: 's', type: 'shape' } as PptxElement)).toBeNull();
		expect(
			buildSmartArt3DElementModel(
	...
	it('falls back to the layout engine without a cached drawing', () => {
		const model = buildSmartArt3DElementModel(
			smartArtElement({ layoutType: 'list', nodes } as unknown as PptxSmartArtData),
		);
	...
	it('builds from the cached drawing (its geometry and theme fills) when the element has one', () => {
		...
		const model = buildSmartArt3DElementModel(smartArtElement(data));
