import type { ResolvedOleType } from 'pptx-viewer-shared';

import { createSvgEl } from '../dom';

/**
 * Type-specific OLE placeholder icons, vanilla port of the inline SVG icons in
 * Vue's `OleRenderer.vue` (Excel grid, Word document, PDF, Visio diagram,
 * MathType `f(x)`, and a generic linked-objects glyph). Data-driven so each
 * icon stays a compact shape list; the brand colour is injected at build time.
 */

interface IconShape {
	tag: 'rect' | 'line' | 'text';
	attrs: Record<string, string | number>;
	text?: string;
}

const rect = (x: number, y: number, width: number, height: number, rx: number): IconShape => ({
	tag: 'rect',
	attrs: { x, y, width, height, rx, 'stroke-width': 1.5, fill: 'none' },
});

const line = (
	x1: number,
	y1: number,
	x2: number,
	y2: number,
	strokeWidth = 1,
	round = false,
): IconShape => ({
	tag: 'line',
	attrs: {
		x1,
		y1,
		x2,
		y2,
		'stroke-width': strokeWidth,
		...(round ? { 'stroke-linecap': 'round' } : {}),
	},
});

const text = (
	x: number,
	y: number,
	content: string,
	fontSize: number,
	italic = false,
): IconShape => ({
	tag: 'text',
	attrs: {
		x,
		y,
		'text-anchor': 'middle',
		'font-size': fontSize,
		'font-weight': 'bold',
		...(italic ? { 'font-style': 'italic' } : {}),
	},
	text: content,
});

const ICONS: Record<ResolvedOleType, IconShape[]> = {
	excel: [
		rect(3, 3, 18, 18, 2),
		line(3, 9, 21, 9),
		line(3, 15, 21, 15),
		line(9, 3, 9, 21),
		line(15, 3, 15, 21),
	],
	word: [
		rect(4, 2, 16, 20, 2),
		line(7, 7, 17, 7, 1.5, true),
		line(7, 11, 17, 11, 1.5, true),
		line(7, 15, 13, 15, 1.5, true),
	],
	pdf: [rect(4, 2, 16, 20, 2), text(12, 14, 'PDF', 7)],
	visio: [
		rect(8, 2, 8, 5, 1),
		line(12, 7, 12, 10, 1.5),
		line(6, 10, 18, 10, 1.5),
		line(6, 10, 6, 13, 1.5),
		line(18, 10, 18, 13, 1.5),
		rect(2, 13, 8, 5, 1),
		rect(14, 13, 8, 5, 1),
	],
	mathtype: [rect(2, 4, 20, 16, 2), text(12, 15, 'f(x)', 9, true)],
	unknown: [rect(2, 5, 9, 7, 1.5), rect(13, 12, 9, 7, 1.5), line(11, 8.5, 13, 15.5, 1.5, true)],
};

/** Build the 36x36 placeholder icon `<svg>` for a resolved OLE type. */
export function buildOleIcon(doc: Document, type: ResolvedOleType, color: string): SVGSVGElement {
	const svg = createSvgEl(doc, 'svg', {
		width: 36,
		height: 36,
		viewBox: '0 0 24 24',
		fill: 'none',
		'aria-hidden': 'true',
	});
	for (const shape of ICONS[type]) {
		const node = createSvgEl(doc, shape.tag, shape.attrs);
		if (shape.tag === 'text') {
			node.setAttribute('fill', color);
			node.textContent = shape.text ?? '';
		} else {
			node.setAttribute('stroke', color);
		}
		svg.appendChild(node);
	}
	return svg;
}
